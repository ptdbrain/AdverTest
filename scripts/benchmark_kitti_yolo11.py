"""Benchmark YOLO11 on KITTI under the group C (occlusion & sensor fault) attacks.
    ...
    python scripts/benchmark_kitti_yolo11.py --root data/anonymized/kitti --limit 500
...
bootstrap confidence intervals and the sanity-check table. The input root must
be an anonymized KITTI export produced by ``anonymize-dataset``.
...
from src.core.hashing import clean_key, file_digest, variant_key
...
def _provenance(args: argparse.Namespace, config: RunConfig) -> dict[str, Any]:
    """Record the immutable inputs needed to reproduce the benchmark."""
    checkpoint = Path(args.weights).expanduser()
    root = Path(config.dataset_params.get("root", ""))
    ...
    return {
        ...
        "dataset_manifest_hash": file_digest(manifest) if manifest.is_file() else None,
        "dataset_descriptor_hash": file_digest(descriptor) if descriptor.is_file() else None,
    }
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--root",
        default=None,
        help="anonymized KITTI root (default: $ADVERTEST_KITTI_ROOT)",
    )
    parser.add_argument(
        "--model",
        default="yolo11",
    ...
    parser.add_argument("--weights", default="checkpoints/surrogates/yolo11s.pt")
    ...
    parser.add_argument(
        "--raw-root",
        default=None,
        help="optional raw KITTI root for an explicit anonymization-cost diagnostic",
    )
    parser.add_argument(
        "--anonymization-delta",
        action="store_true",
        help="measure raw versus anonymized AP; requires --raw-root",
    )
...
def _anonymisation_delta(
    ...
    """AP lost to anonymization itself, only when a raw root is explicit.

    This calls the adapter directly instead of going through :class:`TestRunner`.
    The diagnostic is separate from benchmark generation and is never used as an
    attack dataset. The normal benchmark path always uses the anonymized root.
    """
    if not args.raw_root:
        raise ValueError("--anonymization-delta requires --raw-root")
    raw_params = {**config.dataset_params, "root": args.raw_root, "anonymize": "off"}
    ...
    return {
        ...
        "note": "explicit raw-versus-anonymized diagnostic; raw data is not a benchmark input",
    }
...
def _markdown(report: RunReport, payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "> Input is an anonymized KITTI export. Results are benchmark evidence, not",
        "> a deployment decision; report checkpoint and dataset provenance with them.",
        "",
        f"- model: `{report.model_version}`",
        f"- dataset: `{report.dataset}`, {report.n_samples} frames",
        f"- checkpoint hash: `{payload['provenance']['checkpoint_hash'] or 'unavailable'}`",
        f"- dataset manifest hash: `{payload['provenance']['dataset_manifest_hash'] or 'unavailable'}`",
    ...
    if payload["anonymization"]:
        ...
        lines += [
            ...
            f"- AP after anonymization: {anon['ap_anonymized']:.4f}",

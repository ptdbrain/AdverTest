...
def summarize_batch(batch: dict[str, Any]) -> ValidationSummary:
    ...
    if not batch.get("anonymized", False):
        issues.append(
            _issue(
                "ANONYMISATION_REQUIRED", "anonymized", None, "Benchmark datasets require an anonymisation manifest."
            )
        )
    ...
    if batch.get("dataset_kind") == "attacked_paired":
        ...
        if not manifest.get("clean_dataset_version_id") or not manifest.get("pairs"):
            issues.append(
                _issue(
                    "PAIRED_MANIFEST_REQUIRED",
                    "attacked_manifest",
                    None,
                    "Paired attacked data requires a clean dataset version and sample mapping.",
                )
            )
        for required in ("attack_name", "attack_version", "severity", "seed", "source_hash", "ground_truth_hash"):
            if manifest.get(required) is None:
                issues.append(
                    _issue(
                        "PAIRED_MANIFEST_INCOMPLETE",
                        f"attacked_manifest.{required}",
                        None,
                        "Paired attacked data requires complete attack provenance and hashes.",
                    )
                )
        ...
        for index, pair in enumerate(manifest.get("pairs", [])):
            ...
            if not clean_id or not attacked_id or attacked_id not in known_sample_ids:
                issues.append(
                    _issue(
                        "PAIRED_SAMPLE_UNMAPPED",
                        f"attacked_manifest.pairs.{index}",
                        None,
                        "Every paired manifest entry must map one uploaded attacked sample to one clean sample ID.",
                    )
                )
            elif attacked_id in mapped_attacked_ids:
                issues.append(
                    _issue(
                        "PAIRED_SAMPLE_DUPLICATE",
                        f"attacked_manifest.pairs.{index}",
                        attacked_id,
                        "An attacked sample may appear in only one clean pair.",
                    )
                )
        ...
        for sample_id in sorted(known_sample_ids - mapped_attacked_ids):
            issues.append(
                _issue(
                    "PAIRED_SAMPLE_UNMAPPED",
                    "attacked_manifest.pairs",
                    sample_id,
                    "Each uploaded attacked sample must have a clean pair.",
                )
            )
    ...
    for sample_id, sample in samples.items():
        ...
        if not stored:
            issues.append(
                _issue("ANNOTATION_REQUIRED", "annotation", sample_id, "A benchmark sample needs a valid annotation.")
            )
            continue
        try:
            ...
            issues.append(
                _issue(
                    "ANNOTATION_DOCUMENT_INVALID",
                    "annotation",
                    sample_id,
                    "Annotation document does not match the canonical schema.",
                )
            )
            continue
        if document.task_id != task_id:
            issues.append(
                _issue(
                    "TASK_ANNOTATION_MISMATCH",
                    "annotation.task_id",
                    sample_id,
                    "Annotation task does not match the upload batch.",
                )
            )
            continue
...
def _validate_boxes(
    document: AnnotationDocument, sample_id: str, sample: dict[str, Any], class_map: dict[str, str]
) -> list[ValidationIssue]:
    issues: list[ValidationIssue] = []
    ...
    for index, annotation in enumerate(document.annotations):
        ...
        if not isinstance(coords, list) or len(coords) != 4 or not all(_finite(value) for value in coords):
            issues.append(
                _issue(
                    "BBOX_INVALID",
                    f"annotations.{index}.bbox_xyxy",
                    sample_id,
                    "bbox_xyxy must contain four finite coordinates.",
                )
            )
            continue
        ...
        if x1 >= x2 or y1 >= y2:
            ...
            issues.append(
                _issue(
                    ...
                    f"annotations.{index}.bbox_xyxy",
                    sample_id,
                    "Bounding box coordinates must be inside the original image.",
                )
            )
    return issues
    ...
    if not document.annotations:
        return [
            _issue(
                ...
                "annotations",
                sample_id,
                "Segmentation requires at least one labelled polygon or mask.",
            )
        ]
    width, height = sample.get("width"), sample.get("height")
    for index, annotation in enumerate(document.annotations):
            ...
            if not isinstance(point, list) or len(point) != 2 or not all(_finite(value) for value in point):
                issues.append(
                    _issue(
                        ...
                        f"annotations.{index}.polygon",
                        sample_id,
                        "Polygon points must be finite [x, y] pairs.",
                    )
                )
                break
            ...
            if width is None or height is None or x < 0 or y < 0 or x > float(width) or y > float(height):
                issues.append(
                    _issue(
                        ...
                        f"annotations.{index}.polygon",
                        sample_id,
                        "Polygon points must remain inside the original image.",
                    )
                )
                break
    return issues
...
def _validate_boxes3d(document: AnnotationDocument, sample_id: str, class_map: dict[str, str]) -> list[ValidationIssue]:
    ...
    if not document.calibration:
        issues.append(
            _issue("CALIBRATION_REQUIRED", "calibration", sample_id, "3D detection requires camera calibration.")
        )
    if not document.lidar_path or ".." in document.lidar_path.replace("\\", "/").split("/"):
        issues.append(
            _issue("LIDAR_REQUIRED", "lidar_path", sample_id, "3D detection requires a dataset-relative LiDAR file.")
        )
    if not document.annotations:
    ...
    for index, annotation in enumerate(document.annotations):
        ...
        if not isinstance(box, list) or len(box) != 7 or not all(_finite(value) for value in box):
            issues.append(
                _issue(
                    "BOX3D_INVALID", f"annotations.{index}.box3d", sample_id, "box3d must contain seven finite values."
                )
            )
    return issues
...
def _validate_class(
    annotation: dict[str, Any], index: int, sample_id: str, class_map: dict[str, str], issues: list[ValidationIssue]
) -> None:
    ...
    if not isinstance(class_id, str) or class_id not in class_map:
        issues.append(
            _issue(
                "CLASS_NOT_IN_MAP",
                f"annotations.{index}.class_id",
                sample_id,
                "Annotation class must exist in the batch class map.",
            )
        )

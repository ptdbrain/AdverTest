...
class PlatformCheckpointService:
    ...
    def list(self, project_id: str) -> list[dict[str, Any]]:
        """Return only checkpoints owned by ``project_id`` with their run gate."""
        with self._database.session() as session:
            rows = session.execute(
                select(CheckpointRecord, ArtifactRecord)
                .join(ArtifactRecord, ArtifactRecord.id == CheckpointRecord.artifact_id)
                .where(CheckpointRecord.project_id == project_id)
                .order_by(CheckpointRecord.created_at.desc())
            ).all()
            return [_checkpoint_payload(checkpoint, artifact) for checkpoint, artifact in rows]
...
def _checkpoint_payload(record: CheckpointRecord | None, artifact: ArtifactRecord | None = None) -> dict[str, Any]:
    if record is None:
        raise KeyError("CHECKPOINT_UNKNOWN")
    family = FAMILIES.get(record.model_family_id)
    runnable = bool(
        record.status == CheckpointStatus.READY.value
        and (artifact is None or artifact.state == ArtifactState.READY.value)
        and family
        and family.runnable
    )
    blocked_reason = None
    if not runnable:
        if record.status != CheckpointStatus.READY.value:
            blocked_reason = record.validation_error_code or record.status
        elif artifact is not None and artifact.state != ArtifactState.READY.value:
    ...
    return {
        ...
        "display_name": artifact.original_filename if artifact is not None else record.id,

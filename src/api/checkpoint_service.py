...
_CHECKPOINT_TRANSITIONS = {
    ...
    CheckpointStatus.INTEGRITY_VALIDATING.value: {CheckpointStatus.SANDBOX_LOADING.value, CheckpointStatus.REJECTED.value},
    CheckpointStatus.SANDBOX_LOADING.value: {CheckpointStatus.METADATA_EXTRACTING.value, CheckpointStatus.FAILED_VALIDATION.value},
    CheckpointStatus.METADATA_EXTRACTING.value: {CheckpointStatus.SMOKE_TESTING.value, CheckpointStatus.FAILED_VALIDATION.value},
...
class PlatformCheckpointService:
        ...
        *,
        ...
        with self._database.session() as session:
            artifact = session.scalar(select(ArtifactRecord).where(
                ArtifactRecord.id == artifact_id, ArtifactRecord.project_id == project_id,
            ))
            if artifact is None:
            ...
            checkpoint = CheckpointRecord(
                id=checkpoint_id, project_id=project_id, created_by_user_id=actor_id, artifact_id=artifact_id,
                task_id=task_id, model_family_id=model_family_id, source=source.value,
                status=CheckpointStatus.QUARANTINED.value,
            )
            session.add(checkpoint)
        job = self._jobs.create(
            project_id=project_id, owner_user_id=actor_id, job_type="checkpoint_validation",
            request={"checkpoint_id": checkpoint_id}, total_units=6,
        )
    ...
    def request_ultralytics_import(self, *, project_id: str, actor_id: str, model_id: str) -> dict[str, Any]:
        ...
        return self._jobs.create(
            project_id=project_id, owner_user_id=actor_id, job_type="ultralytics_import",
            request={"model_id": model_id}, total_units=7,
            idempotency_key=f"ultralytics-import:{model_id}",
        )

    def get(self, project_id: str, checkpoint_id: str) -> dict[str, Any] | None:
        with self._database.session() as session:
            checkpoint = session.scalar(select(CheckpointRecord).where(
                CheckpointRecord.id == checkpoint_id, CheckpointRecord.project_id == project_id,
            ))
    ...
    def validation_history(self, project_id: str, checkpoint_id: str) -> list[dict[str, Any]]:
        with self._database.session() as session:
            ...
            return [{
                "id": item.id, "stage": item.stage, "passed": item.passed, "error_code": item.error_code,
                "details": json.loads(item.details_json), "created_at": item.created_at,
            } for item in records]
    ...
    def validate_job(self, job_id: str, request: dict[str, Any], progress: Callable[..., bool]) -> dict[str, Any]:
        ...
        result = self._inspect_checkpoint(artifact_bytes)
        ...
        self._record_validation(checkpoint_id, job_id, "SMOKE_TESTING", True, None, {"output_contract": result.get("output_contract")})

        self._transition(checkpoint_id, CheckpointStatus.READY, job_id, True, None, {"class_schema_hash": class_schema_hash})
    ...
    def import_ultralytics_job(self, job_id: str, project_id: str, actor_id: str, request: dict[str, Any], progress: Callable[..., bool]) -> dict[str, Any]:
        ...
        artifact = self._artifacts.create_internal(
            ...
            original_filename=f"{model_id}.pt", mime_type="application/octet-stream",
            content=checkpoint_bytes, metadata={"source": "ultralytics", "model_id": model_id},
        )
        checkpoint, validation_job = self.register_uploaded(
            project_id=project_id, actor_id=actor_id, artifact_id=artifact["id"], task_id="detection2d",
            model_family_id="yolo11", source=CheckpointSource.ULTRALYTICS_IMPORT,
        )
    ...
    def _inspect_checkpoint(self, artifact_bytes: bytes) -> dict[str, Any]:
    ...
    def _checkpoint_for_worker(self, checkpoint_id: str) -> dict[str, Any]:
        with self._database.session() as session:
            ...
            return {**_checkpoint_payload(checkpoint), "artifact_sha256": artifact.sha256, "original_filename": artifact.original_filename}

    def _transition(
        self, checkpoint_id: str, target: CheckpointStatus, job_id: str, passed: bool | None, error_code: str | None, details: dict[str, Any]
    ) -> None:
        with self._database.session() as session:
            checkpoint = session.get(CheckpointRecord, checkpoint_id)
            if checkpoint is None:
            ...
            session.add(CheckpointValidationRecord(
                id=str(uuid4()), checkpoint_id=checkpoint_id, job_id=job_id, stage=target.value,
                passed=passed, error_code=error_code, details_json=json.dumps(details, sort_keys=True),
            ))

    def _record_validation(self, checkpoint_id: str, job_id: str, stage: str, passed: bool, error_code: str | None, details: dict[str, Any]) -> None:
        with self._database.session() as session:
            session.add(CheckpointValidationRecord(
                id=str(uuid4()), checkpoint_id=checkpoint_id, job_id=job_id, stage=stage,
                passed=passed, error_code=error_code, details_json=json.dumps(details, sort_keys=True),
            ))
    ...
    def _reject(self, checkpoint_id: str, job_id: str, error_code: str, details: dict[str, Any]) -> None:
        with self._database.session() as session:
            ...
            if checkpoint is None:
                return
            terminal = CheckpointStatus.REJECTED if checkpoint.status == CheckpointStatus.INTEGRITY_VALIDATING.value else CheckpointStatus.FAILED_VALIDATION
            if terminal.value in _CHECKPOINT_TRANSITIONS.get(checkpoint.status, set()):
                checkpoint.status = terminal.value
            checkpoint.validation_error_code = error_code
            session.add(CheckpointValidationRecord(
                id=str(uuid4()), checkpoint_id=checkpoint_id, job_id=job_id, stage=checkpoint.status,
                passed=False, error_code=error_code, details_json=json.dumps(details, sort_keys=True),
            ))
...
def _inspect_in_sandbox(content: bytes, *, timeout_seconds: int, memory_mb: int, cpu_seconds: int) -> dict[str, Any]:
    context = multiprocessing.get_context("spawn")
    result_queue: Any = context.Queue(maxsize=1)
    process = context.Process(target=_sandbox_child, args=(content, memory_mb, cpu_seconds, result_queue), daemon=True)
    process.start()
...
def _inspect_with_external_sandbox(url: str, token: str | None, content: bytes, *, timeout_seconds: int) -> dict[str, Any]:
...
def _sandbox_child(content: bytes, memory_mb: int, cpu_seconds: int, result_queue: Any) -> None:
    try:
        import resource
        import socket
        ...
        with tempfile.TemporaryDirectory(prefix="advertest-checkpoint-") as directory:
            ...
            result_queue.put({"ok": True, "class_names": names, "smoke_tested": bool(prediction), "output_contract": "ultralytics-results-v1"})
    except Exception as exc:
        result_queue.put({"ok": False, "error_code": "CHECKPOINT_RUNTIME_FAILED", "detail": f"{type(exc).__name__}: {exc}"})
...
def _checkpoint_payload(record: CheckpointRecord | None) -> dict[str, Any]:
    if record is None:
        raise KeyError("CHECKPOINT_UNKNOWN")
    return {
        "id": record.id, "project_id": record.project_id, "created_by_user_id": record.created_by_user_id,
        "artifact_id": record.artifact_id, "task_id": record.task_id, "model_family_id": record.model_family_id,
        "source": record.source, "status": record.status,
        "native_class_names": json.loads(record.native_class_names_json), "num_classes": record.num_classes,
        "class_schema_hash": record.class_schema_hash, "validation_error_code": record.validation_error_code,
...
class CheckpointValidationService:
    ...
    def _validate(self, checkpoint_id: str, job_id: str) -> None:
        ...
        record["validation"] = {"state": "READY", "reason": None, "capability_confirmation": "quarantine_integrity_verified"}
        self._records.update_record("checkpoint", checkpoint_id, record)

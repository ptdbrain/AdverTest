...
class ModelVersionOut(BaseModel):
    ...
    # Native output classes for the UI mapping matrix (family-level default).
    native_class_names: list[str] = Field(default_factory=list)
...
class SampleReviewUpsertIn(BaseModel):
    """One per-sample review decision (workflow step 12: duyệt từng ảnh)."""

    sample_id: str = Field(min_length=1, max_length=256)
    attack: str = Field(min_length=1, max_length=128)
...
class SampleReviewBatchIn(BaseModel):
    """Batch upsert of per-sample review decisions for one run."""

    reviews: list[SampleReviewUpsertIn] = Field(min_length=1)


class SampleReviewOut(BaseModel):
    """Persisted per-sample review decision."""
...
class AdversarialDatasetOut(BaseModel):
    """Metadata of one reviewer-approved adversarial dataset export."""
    ...
    class_map: dict[str, str] = Field(default_factory=dict)
    format: str = "yolo"
    filename: str = "adversarial_dataset.zip"
    sha256: str = ""
    download_url: str
    created_at: str = ""


class AttackConfigItemIn(BaseModel):
    """One attack in a named, saved attack configuration."""
...
class AttackConfigIn(BaseModel):
    """Create/update request for a named attack configuration (workflow step 5)."""

    name: str = Field(min_length=1, max_length=120)
    task_id: str = Field(min_length=1)
    items: list[AttackConfigItemIn] = Field(min_length=1)


class AttackConfigOut(BaseModel):
    """One named attack configuration, reusable across experiments."""

    config_id: str
    project_id: str
    name: str
    task_id: str
    items: list[AttackConfigItemIn] = Field(default_factory=list)
    created_at: str = ""
    updated_at: str = ""

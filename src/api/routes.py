...
from src.api.helpers import artifact_uri as _local_artifact_uri
...
from src.demo_fake_bootstrap import fixture_for_baseline
...
from src.models.families import FAMILIES, adapter_request, approved_model_config, family_for_version
from src.models.versions import ModelVersion
from src.persistence.models import SessionRunRecord
...
def _version_out(version: ModelVersion) -> ModelVersionOut:
    """ModelVersionOut enriched with the family's native class names.

    Platform-registered checkpoints carry their own class metadata; locally
    scanned/demo checkpoints fall back to the family's reference classes so
    the UI mapping matrix shows the checkpoint's real label space.
    """
    payload = ModelVersionOut.from_domain(version)
    family = FAMILIES.get(version.model_family_id or "")
    if family is not None and family.default_class_names:
        payload.native_class_names = list(family.default_class_names)
    return payload
...
async def list_model_versions() -> list[ModelVersionOut]:
    ...
    return [_version_out(version) for version in _registered_model_versions()]
...
async def list_perception_modes() -> list[PerceptionModeOut]:
    """Product-facing mode availability, including honest gates for each modality."""
    versions = _registered_model_versions()
    sam = next((version for version in versions if version.task == "segmentation"), None)
    pp = next((version for version in versions if version.task == "detection3d"), None)
    return [
        PerceptionModeOut(
            id="detection2d",
        ...
        PerceptionModeOut(
            ...
            status="ready" if sam and sam.runnable else "waiting_for_artifacts",
        ),
        PerceptionModeOut(
            id="detection3d",
            ...
            metric_labels=(
                "Clean BEV AP",
                "BEV AP After Attack",
                "BEV Performance Lost",
                "Mean BEV IoU",
                "Objects Broken by Attack",
            ),
            runnable=bool(pp and pp.runnable),
            blocked_reason=None if pp and pp.runnable else "WAITING_FOR_ARTIFACTS",
            status="ready" if pp and pp.runnable else "waiting_for_artifacts",
        ),
    ]
...
async def create_defence_run(
    body: DefenceRunIn, project_id: str | None = Query(default=None)
) -> RunJobOut:
    ...
    _ensure_run_project_scope(baseline, project_id)
    ...
    try:
        fake_fixture = fixture_for_baseline(baseline)
    except ValueError as exc:
        raise HTTPException(status_code=500, detail={"code": str(exc)}) from exc
    if fake_fixture is not None:
        if candidate.id != fake_fixture.defence_checkpoint_id:
            raise HTTPException(status_code=422, detail={"code": "DEMO_DEFENCE_CANDIDATE_MISMATCH"})
        prepared = _store.get(fake_fixture.defence_run_id)
        if prepared is None or prepared.get("status") != "COMPLETED" or prepared.get("report") is None:
            raise HTTPException(status_code=503, detail={"code": "DEMO_DEFENCE_NOT_PREPARED"})
        return _job_out(prepared)
...
async def run_defence_candidates(
    run_id: str, project_id: str | None = Query(default=None)
) -> list[ModelVersionOut]:
    """Fine-tuned/repaired weights are visible only after an attack run."""
    run = _require_run(run_id)
    _ensure_run_project_scope(run, project_id)
...
async def get_report(run_id: str, project_id: str | None = Query(default=None)) -> RunReportOut:
    item = _require_run(run_id)
    _ensure_run_project_scope(item, project_id)
...
async def download_run_zip(run_id: str, project_id: str | None = Query(default=None)) -> Response:
...
async def download_run_pdf(run_id: str, project_id: str | None = Query(default=None)) -> Response:
    """Render the benchmark report as a portable PDF executive summary."""
    ...
    if report is None:
        raise HTTPException(status_code=409, detail="run evidence is not available until the run completes")
    return Response(
        content=generate_run_report_pdf(report, run_id),
...
async def auto_flag_reviews(
    run_id: str, threshold: float = Query(default=30.0), project_id: str | None = Query(default=None)
) -> dict[str, Any]:
    """Auto-create review items for cells exceeding degradation threshold."""
    _ensure_run_project_scope(_require_run(run_id), project_id)
...
@router.get("/runs/{run_id}/sample-reviews", response_model=list[SampleReviewOut])
async def list_sample_reviews(
    run_id: str,
    decision: str | None = Query(default=None),
    project_id: str | None = Query(default=None),
    actor_id: str | None = Depends(require_run_project_member),
) -> list[SampleReviewOut]:
    """Per-sample review decisions saved for one run (workflow step 12)."""
    del actor_id
    _require_review_run_scope(run_id, project_id)
    _require_run(run_id)
    return [SampleReviewOut(**row) for row in _store.list_sample_reviews(run_id, decision=decision)]


@router.put("/runs/{run_id}/sample-reviews", response_model=list[SampleReviewOut])
async def upsert_sample_reviews(
    run_id: str,
    body: SampleReviewBatchIn,
    project_id: str | None = Query(default=None),
    actor_id: str | None = Depends(require_run_project_member),
) -> list[SampleReviewOut]:
    """Batch approve/reject individual attacked samples of one run."""
    del actor_id
    _require_review_run_scope(run_id, project_id)
    _require_run(run_id)
    try:
        saved = _store.upsert_sample_reviews(run_id, [entry.model_dump() for entry in body.reviews])
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=str(exc.args[0])) from exc
    return [SampleReviewOut(**row) for row in saved]


def _adversarial_dataset_record_path(dataset_id: str) -> Path:
    settings = get_settings()
    return Path(settings.artifact_root).expanduser().resolve() / "adversarial-datasets" / f"{dataset_id}.zip"


def _require_review_run_scope(run_id: str, project_id: str | None) -> None:
    """Ensure a supplied product project owns the session record for this run."""
    if not project_id:
        return
    with get_platform_database().session() as db:
        linked = (
            ...
            .filter(
                SessionRunRecord.project_id == project_id,
                (SessionRunRecord.id == run_id) | (SessionRunRecord.backend_run_id == run_id),
            )
            .first()
        )
    if linked is None:
        raise HTTPException(status_code=404, detail="RUN_NOT_IN_PROJECT")


def _adversarial_dataset_payload(store, dataset_id: str) -> dict[str, Any] | None:
    for record in store.list_records("adversarial_dataset"):
        if record.get("id") == dataset_id or record.get("dataset_id") == dataset_id:
            payload = dict(record)
...
@router.post("/runs/{run_id}/adversarial-dataset", response_model=AdversarialDatasetOut, status_code=201)
async def create_adversarial_dataset(
    run_id: str,
    project_id: str | None = Query(default=None),
    actor_id: str | None = Depends(require_run_project_member),
) -> AdversarialDatasetOut:
    """Build the task-native ZIP from reviewer-approved samples (workflow step 13)."""
    del actor_id
    _require_review_run_scope(run_id, project_id)
    run = _require_run(run_id)
    report = run["report"]
    if report is None:
        raise HTTPException(status_code=409, detail="sample evidence is not available until the run completes")
    approved = _store.list_sample_reviews(run_id, decision="APPROVED")
    if not approved:
        raise HTTPException(status_code=409, detail="APPROVED_SAMPLES_REQUIRED: approve at least one image first")

    keys = {(row["sample_id"], row["attack"], row["severity"]) for row in approved}
    items = [
        sample
        for sample in report.get("sample_results", [])
        if (sample.get("sample_id"), sample.get("attack"), sample.get("severity")) in keys
    ]
    if not items:
        raise HTTPException(status_code=409, detail="approved samples no longer have stored evidence")

    task_id = str((run.get("config") or {}).get("task_id") or "detection2d")
    if task_id == "segmentation":
        from src.training.segmentation_dataset_formatter import build_attacked_segmentation_zip

        zip_bytes, manifest = build_attacked_segmentation_zip(items, run_id=run_id)
    elif task_id == "detection3d":
        from src.training.kitti3d_dataset_formatter import build_attacked_kitti3d_zip

        zip_bytes, manifest = build_attacked_kitti3d_zip(items, run_id=run_id)
    else:
        from src.training.yolo_dataset_formatter import build_attacked_yolo_zip

        zip_bytes, manifest = build_attacked_yolo_zip(items, run_id=run_id)
    if manifest["sample_count"] == 0:
        raise HTTPException(
            status_code=409,
            detail={
                "code": "EXPORT_HAS_NO_VALID_SAMPLES",
                "task_id": task_id,
                "skipped": manifest["skipped"],
            },
        )
    dataset_id = f"ads-{stable_digest({'run_id': run_id, 'manifest': manifest['samples']}, length=24)}"
    output_path = _adversarial_dataset_record_path(dataset_id)
    ...
    payload = {
        ...
        "filename": "adversarial_dataset.zip",
        "sha256": hashlib.sha256(zip_bytes).hexdigest(),
        "created_at": manifest["generated_at"],
    }
    _store.put_record("adversarial_dataset", dataset_id, payload)
    return AdversarialDatasetOut(
        **payload,
        download_url=(
            f"/api/v1/runs/{run_id}/adversarial-datasets/{dataset_id}/download"
            + (f"?project_id={project_id}" if project_id else "")
        ),
    )


@router.get("/runs/{run_id}/adversarial-datasets/{dataset_id}/download")
async def download_adversarial_dataset(
    run_id: str,
    dataset_id: str,
    project_id: str | None = Query(default=None),
    actor_id: str | None = Depends(require_run_project_member),
) -> Response:
    """Stream a previously generated adversarial dataset ZIP."""
    del actor_id
    _require_review_run_scope(run_id, project_id)
    payload = _adversarial_dataset_payload(_store, dataset_id)
    if (
        payload is None
        or payload.get("run_id") != run_id
        or (project_id is not None and payload.get("project_id") != project_id)
    ):
        raise HTTPException(status_code=404, detail="unknown adversarial dataset")
    output_path = _adversarial_dataset_record_path(dataset_id)
    if not output_path.is_file():
        raise HTTPException(status_code=410, detail="adversarial dataset archive is no longer stored")
    return Response(
        content=output_path.read_bytes(),
        media_type="application/zip",
        headers={"Content-Disposition": f'attachment; filename="{payload.get("filename", "adversarial_dataset.zip")}"'},
    )


def _attack_config_payload(row: dict[str, Any]) -> AttackConfigOut:
    payload = json.loads(row.get("payload") or "{}")
    return AttackConfigOut(
        config_id=row["id"],
...
async def require_project_member_by_config(
    config_id: str, current_user=Depends(get_current_user)
) -> str:
    """Authorize named attack-config routes from their persisted project owner."""
    row = _store.get_attack_config(config_id)
    if row is None:
        raise HTTPException(status_code=404, detail="unknown attack config")
    assert_project_member(row["project_id"], current_user.id)
    return current_user.id


@router.get("/attack-configs", response_model=list[AttackConfigOut])
async def list_attack_configs(
    project_id: str = Query(...), actor_id: str = Depends(require_project_member)
) -> list[AttackConfigOut]:
    """Named attack configurations saved for a project (workflow step 5)."""
    del actor_id
    return [_attack_config_payload(row) for row in _store.list_attack_configs(project_id)]


@router.post("/attack-configs", response_model=AttackConfigOut, status_code=201)
async def create_attack_config(
    body: AttackConfigIn, project_id: str = Query(...), actor_id: str = Depends(require_project_member)
) -> AttackConfigOut:
    del actor_id
    config_id = f"afc-{stable_digest({'project_id': project_id, 'name': body.name}, length=16)}"
    row = _store.upsert_attack_config(config_id, project_id, body.name, body.task_id, {"items": [item.model_dump() for item in body.items]})
    return _attack_config_payload(row)


@router.get("/attack-configs/{config_id}", response_model=AttackConfigOut)
async def get_attack_config(config_id: str, actor_id: str = Depends(require_project_member_by_config)) -> AttackConfigOut:
    row = _store.get_attack_config(config_id)
    if row is None:
        raise HTTPException(status_code=404, detail="unknown attack config")
    return _attack_config_payload(row)
...
@router.put("/attack-configs/{config_id}", response_model=AttackConfigOut)
async def update_attack_config(
    config_id: str, body: AttackConfigIn, actor_id: str = Depends(require_project_member_by_config)
) -> AttackConfigOut:
    row = _store.get_attack_config(config_id)
    if row is None:
        raise HTTPException(status_code=404, detail="unknown attack config")
    del actor_id
    updated = _store.upsert_attack_config(config_id, row["project_id"], body.name, body.task_id, {"items": [item.model_dump() for item in body.items]})
    return _attack_config_payload(updated)


@router.delete("/attack-configs/{config_id}", status_code=204)
async def delete_attack_config(config_id: str, actor_id: str = Depends(require_project_member_by_config)) -> Response:
    del actor_id
    if not _store.delete_attack_config(config_id):
        raise HTTPException(status_code=404, detail="unknown attack config")
...
async def list_reviews(
    status: str | None = Query(default=None), project_id: str | None = Query(default=None)
) -> list[ReviewOut]:
    rows = _store.list_reviews(status=status, project_id=project_id)
...
async def resolve_review(
    review_id: str, body: ResolveReviewIn, project_id: str | None = Query(default=None)
) -> ReviewOut:
    ...
    if existing is None:
        raise HTTPException(status_code=404, detail=f"unknown review {review_id!r}")
    if project_id:
        _ensure_run_project_scope(_require_run(existing["run_id"]), project_id)
    success = _store.resolve_review(review_id, body.decision, body.decision_note, body.resolved_by)
    if not success:
        raise HTTPException(status_code=404, detail=f"unknown review {review_id!r}")
    row = _store.get_review(review_id)
    if row:
        trigger_downstream_actions(body.decision, row, _store, _workflow_store)
...
async def auto_group_failure_clusters(run_id: str | None = Query(default=None)) -> list[dict[str, Any]]:
    ...
    pending_reviews = _store.list_reviews(status="PENDING")
    if run_id:
        pending_reviews = [r for r in pending_reviews if r.get("run_id") == run_id]

    if not pending_reviews:
        return []

    clusterer = SmartFailureClusterer(max_clusters=8, distance_threshold=0.4)
    clusters = clusterer.cluster(pending_reviews)
    ...
    for c in clusters:
        payload = {
            ...
            "name": c.label,
            "label": c.label,
            "member_ids": list(c.member_review_ids),
            "member_review_ids": list(c.member_review_ids),
            "member_count": c.member_count,
            "representative_review_id": c.representative_review_id,
            "mean_degradation": c.mean_degradation,
            "max_degradation": c.max_degradation,
            "severity_distribution": c.severity_distribution,
            "affected_classes": list(c.affected_classes),
            "dominant_attack_group": c.dominant_attack_group,
            "dominant_risk_level": c.dominant_risk_level,
            "recommended_action": c.recommended_action,
            "selection_allowed": True,
        }
        _store.put_record("failure_cluster", c.cluster_id, payload)
    ...
    if raw_value.startswith(("https://", "http://")):
        ...
        if (
            ...
            try:
                ...
                return raw_value
        return raw_value
    return _local_artifact_uri(raw_value)
...
def _registered_model_versions() -> list[Any]:
    ...
    if not portable_enabled:
        ...
        for base in catalog_model_versions(platform=settings.run_execution_backend == "platform"):
            versions[base.id] = base
...
def _resolve_run_config(config: RunConfig, *, allow_defence: bool = False) -> RunConfig:
    ...
    if version.checkpoint_path:
        ...
        if not path.is_file() and not is_remote_catalog:
            raise HTTPException(
...
def _ensure_run_project_scope(item: dict[str, Any], project_id: str | None) -> None:
    if project_id and (item.get("config") or {}).get("project_id") != project_id:
        raise HTTPException(status_code=404, detail="RUN_NOT_IN_PROJECT")

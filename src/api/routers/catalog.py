...
def _demo_dataset_params(name: str) -> dict[str, object]:
    """Return loader parameters for the reviewed bundle in this environment."""
    runtime = resolve_catalog_dataset(name, get_settings())
    return runtime.as_params() if runtime is not None else {}
...
async def list_datasets(task_id: str | None = None) -> list[DatasetCatalogItem]:
    ...
    for dataset in load_datasets().values():
        ...
        runtime = resolve_catalog_dataset(dataset.name, get_settings())
        params = runtime.as_params() if runtime is not None else {}
        item["dataset_params"] = params
        item["demo"] = False
        item["sample_count"] = runtime.sample_count if runtime and runtime.sample_count is not None else _CATALOG_SAMPLE_COUNTS.get(dataset.name)
        item["demo"] = bool(runtime and runtime.sample_count != _CATALOG_SAMPLE_COUNTS.get(dataset.name))
        if runtime is not None:
            item["anonymized"] = runtime.anonymized
            item["runnable"] = runtime.runnable
            item["blocked_reason"] = runtime.blocked_reason

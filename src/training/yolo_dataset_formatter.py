...
def build_attacked_yolo_zip(
    items: list[dict[str, Any]],
    *,
    run_id: str,
    dataset_name: str = "adversarial-dataset",
) -> tuple[bytes, dict[str, Any]]:
    """Package reviewer-approved attacked samples as a YOLO-format ZIP (workflow step 13).

    Args:
        items: One dict per approved sample result: ``sample_id``, ``attack``,
            ``severity``, ``attacked_image_path`` and ``ground_truth`` (the
            ``{"type": "boxes", "image_width", "image_height", "objects":
            [{"label", "xyxy"}]}`` payload the runner stores). Items that lack a
            readable attacked image or box ground truth are listed in the
            manifest's ``skipped`` section instead of failing the export.

    Returns:
        ``(zip_bytes, manifest)`` — the archive holds ``images/`` + ``labels/``
        in YOLO normalized format plus a ``manifest.json`` recording the class
        map, per-file provenance and every skipped sample.
    """
    import hashlib
    import io
    import json
    ...
    for item in items:
        ...
        image_path = item.get("attacked_image_path") or ""
        image_file = Path(image_path) if image_path else None
        if ground_truth.get("type") != "boxes":
            skipped.append({"sample_id": item.get("sample_id"), "reason": "no 2D box ground truth"})
            continue
        if not image_file or not image_file.is_file():
            skipped.append({"sample_id": item.get("sample_id"), "reason": f"attacked image missing: {image_path}"})
            continue
        try:
            width = float(ground_truth.get("image_width"))
            height = float(ground_truth.get("image_height"))
            objects = ground_truth_objects(item)
            valid_geometry = width > 0 and height > 0 and math.isfinite(width) and math.isfinite(height)
            for obj in objects:
                coordinates = [float(value) for value in obj.get("xyxy", [])]
                valid_geometry = valid_geometry and (
                    len(coordinates) == 4
                    and all(math.isfinite(value) for value in coordinates)
                    and 0 <= coordinates[0] < coordinates[2] <= width
                    and 0 <= coordinates[1] < coordinates[3] <= height
                    and bool(obj.get("label"))
                )
        except (TypeError, ValueError):
            valid_geometry = False
        if not valid_geometry:
            skipped.append({"sample_id": item.get("sample_id"), "reason": "invalid 2D box geometry"})
            continue
        included.append({**item, "image_file": image_file})

    class_names = sorted({str(obj["label"]) for item in included for obj in ground_truth_objects(item)})
    class_ids = {label: index for index, label in enumerate(class_names)}
    ...
    with zipfile.ZipFile(payload, mode="w", compression=zipfile.ZIP_DEFLATED) as archive:
        for index, item in enumerate(included):
            stem = f"{index:06d}_{item['sample_id']}"
            image_member = f"images/{stem}{item['image_file'].suffix.lower() or '.png'}"
            image_bytes = item["image_file"].read_bytes()
            archive.writestr(image_member, image_bytes)
            hashes[image_member] = hashlib.sha256(image_bytes).hexdigest()

            width = int(item["ground_truth"]["image_width"])
            height = int(item["ground_truth"]["image_height"])
            label_lines: list[str] = []
            for obj in ground_truth_objects(item):
                x1, y1, x2, y2 = (float(value) for value in obj["xyxy"])
                class_id = class_ids[str(obj["label"])]
                label_lines.append(
                    f"{class_id} {(x1 + x2) / 2 / width:.6f} {(y1 + y2) / 2 / height:.6f} "
                    f"{(x2 - x1) / width:.6f} {(y2 - y1) / height:.6f}"
                )
            label_member = f"labels/{stem}.txt"
            label_bytes = ("\n".join(label_lines) + "\n").encode("utf-8") if label_lines else b""
            archive.writestr(label_member, label_bytes)
            hashes[label_member] = hashlib.sha256(label_bytes).hexdigest()
            manifest_samples.append(
                {
                    ...
                    "degradation_hint": item.get("degradation_hint"),
                }
            )
        ...
        manifest = {
            ...
            "class_map": {str(class_id): label for label, class_id in sorted(class_ids.items())},
...
def ground_truth_objects(item: dict[str, Any]) -> list[dict[str, Any]]:
    """Box objects of one sample-result item; tolerant of the GT payload variants."""
    ground_truth = item.get("ground_truth") or {}
    return list(ground_truth.get("objects") or [])

...
class ProjectRecord(Base):
    ...
    # Workflow §3: a project pursues exactly one task type; sessions that
    # declare a different task are rejected (TASK_PROJECT_MISMATCH).
    task_type: Mapped[str] = mapped_column(String(100), default="detection2d")
    owner_user_id: Mapped[str] = mapped_column(String(36), index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
...
class ExperimentSessionRecord(Base):
    ...
    # Workflow §4: dataset-class → checkpoint-class mapping chosen in the UI,
    # persisted so it survives refreshes (shape: {dataset_class: model_class|null|"ignore"}).
    class_mapping_json: Mapped[str] = mapped_column(Text, default="{}")

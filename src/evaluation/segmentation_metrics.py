...
def sample_pixel_accuracy(sample: Sample, prediction: SegmentationPrediction) -> float | None:
    """Overall pixel accuracy for one sample (label-map level, background incl.).

    Instances are painted onto an empty background (label 0) and compared
    against the ground-truth label map; unassigned pixels count as background.
    Returns ``None`` when the sample carries no instance ground-truth mask.
    """
    target_mask = getattr(sample, "mask", None)
    if target_mask is None:
        return None
    truth = np.asarray(target_mask)
    predicted = np.zeros(truth.shape, dtype=np.int64)
    for instance in prediction.instances:
        mask = np.asarray(instance.mask, dtype=bool)
        if mask.shape != truth.shape:
            return None
        predicted[mask] = int(instance.instance_id)
    total = truth.size
    return float((predicted == truth).sum() / total) if total else None
...
def segmentation_metric_suite(
    ...
    *,
    ...
    # Overall pixel accuracy is a per-sample, label-map level metric — it does
    # not require any prompt-aligned instance evaluation to exist.
    pixel_accuracies = [
        value
        for sample in samples
        if sample.sample_id in by_id
        for value in [sample_pixel_accuracy(sample, by_id[sample.sample_id])]
        if value is not None
    ]
    pixel_accuracy = float(np.mean(pixel_accuracies)) if pixel_accuracies else 0.0
    if not evaluations:
        return {

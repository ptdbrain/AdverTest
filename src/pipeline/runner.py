...
from src.core.memory import trim_memory as _trim_memory
from src.core.types import COST_WEIGHT, ModelInfo, Prediction, Sample, SegmentationPrediction, Task
...
class RunConfig(BaseModel):
    ...
    # Workflow §4 provenance: {dataset_class: model_class | null | "ignore"} as
    # chosen in the mapping matrix. Empty means "no mapping supplied" (legacy
    # runs); a non-empty mapping is validated in the API layer before running.
    class_mapping: dict[str, str | None] = Field(default_factory=dict)
    dataset_class_names: list[str] = Field(default_factory=list)
    severities: list[int] = Field(default_factory=lambda: [1, 3, 5])
    # Independent mode with explicit pairs: when non-empty it replaces the
    # attacks × severities cross-product, keeping the exact (attack, severity)
    # combinations the user configured (e.g. blur@3 + noise@2, not blur@{2,3}).
    attack_severity_pairs: list[tuple[str, int]] = Field(default_factory=list)
    limit: int | None = 8
    seed: int = 20260730
...
class TestRunner:
    ...
    def estimate(self, config: RunConfig) -> CostEstimate:
        ...
        if recipe_steps:
            cell_plan = [
                (get_attack(step.attack_name, **step.parameters), step.severity) for step in recipe_steps
            ]
        elif config.attack_severity_pairs:
            cell_plan = [
                (get_attack(name, **config.attack_params.get(name, {})), severity)
                for name, severity in config.attack_severity_pairs
            ]
        else:
            cell_plan = [
                (get_attack(attack.name, **config.attack_params.get(attack.name, {})), severity)
                for attack in selected
                for severity in config.severities
            ]
        n_cells = 1 if recipe_steps else len(cell_plan)
        queries = sum(attack.model_queries_for_severity(severity) * n_samples for attack, severity in cell_plan)
        gradients = sum(attack.gradient_steps_for_severity(severity) * n_samples for attack, severity in cell_plan)
        units = sum(COST_WEIGHT[attack.cost_class] * n_samples for attack, _ in cell_plan)
    ...
    def preflight(self, config: RunConfig) -> PreflightResult:
        ...
        if config.execution_mode == "benchmark":
            ...
            except AnonymizationRequiredError as exc:
                return PreflightResult((), (), (str(exc),))
        adapter = get_adapter(config.model, **config.adapter_params)
        samples = dataset.load(config.limit)
        ...
        if adapter.metadata().task not in {"detection2d", "segmentation", "detection3d"}:
            fatal.append(
                f"primary TestRunner currently does not support adapter task {adapter.metadata().task!r}"
            )
        ...
        if not config.recipe and config.attack_severity_pairs:
            selected_by_name = {attack.name: attack for attack in selected}
            skipped_by_name = {item.attack: item.reason for item in skipped}
            for name, severity in config.attack_severity_pairs:
                reason = skipped_by_name.get(name)
                if reason:
                    fatal.append(f"INCOMPATIBLE_PAIR: {name}: {reason}")
                    continue
                attack_cls = selected_by_name.get(name)
                if attack_cls is None:
                    fatal.append(f"UNKNOWN_PAIR: {name}")
                elif severity < 0 or severity > attack_cls.severity_levels:
                    fatal.append(
                        f"INVALID_PAIR: {name}: severity {severity} exceeds supported range 0..{attack_cls.severity_levels}"
                    )
        if not config.recipe and (config.attacks or config.attack_severity_pairs) and not selected:
    ...
    def run(
        ...
        *,
        ...
        # Independent mode: explicit pairs keep the exact (attack, severity)
        # combinations the user configured; otherwise run the full cross-product.
        compatible_names = {attack_cls.name for attack_cls in selected}
        if config.attack_severity_pairs:
            pairs_by_attack: dict[str, list[int]] = {}
            for name, severity in config.attack_severity_pairs:
                if name not in compatible_names:
                    continue  # already listed in report.skipped with a reason
                pairs_by_attack.setdefault(name, []).append(severity)
            attack_plan: list[tuple[str, list[int]]] = list(pairs_by_attack.items())
            total_steps = len(config.attack_severity_pairs)
        else:
            attack_plan = [(attack_cls.name, list(config.severities)) for attack_cls in selected]
        ...
        for attack_name, severity_list in attack_plan:
            attack = get_attack(attack_name, **config.attack_params.get(attack_name, {}))
            if progress:
    ...
    def _run_attack(
        ...
        for severity in (severities if severities is not None else config.severities):
            if should_cancel and should_cancel():
            ...
            if adapter.metadata().task == "segmentation":
                ...
                if config.execution_mode == "benchmark":
                    ...
                    metrics["objects_broken"] = attack_success.lost_truths
                    metrics["attack_success_rate"] = attack_success.rate
                cell_ap = average_precision(predictions, samples, config.iou_threshold) if config.execution_mode == "benchmark" else 0.0
            cell = CellResult(
                attack=attack.name,
                group=attack.group,
                severity=severity,
            ...
            for clean_sample, variant, clean_prediction, attacked_prediction in zip(
                ...
                sample_results.append(
                    SampleResult(
                        ...
                        object_evidence=(
                            _segmentation_evidence(clean_sample, attacked_prediction)
                            if adapter.metadata().task == "segmentation"
                            else [
                                detail.as_dict()
    ...
    def _run_recipe(self, recipe: AttackRecipe, samples: Sequence[Sample], clean_predictions: Sequence[Prediction], adapter: ModelAdapter, config: RunConfig, evidence: EvidenceWriter | None, report: RunReport, should_cancel: Callable[[], bool] | None = None) -> None:
        ...
        for sample in samples:
            ...
            results.append(engine.execute(sample, recipe, CompositionContext(run_seed=config.seed, model=adapter, retain_intermediate_arrays=False)))
            _trim_memory()
...
def _incompatibility(attack: type[BaseAttack], dataset: DatasetSource, info: ModelInfo) -> str | None:
    ...
    if dataset.modality != "multi" and attack.modality != dataset.modality:
        return f"attack modality {attack.modality!r} != dataset modality {dataset.modality!r}"
    task_ids = ATTACK_CATALOG.get(attack.name).task_ids
    if task_ids and info.task not in task_ids:
        return f"attack requires model task(s) {sorted(task_ids)!r}; adapter provides {info.task!r}"
    return None
...
def _segmentation_evidence(sample: Sample, prediction: SegmentationPrediction) -> list[dict[str, Any]]:
    """Convert internal mask evaluations to the JSON evidence contract."""
    return [item.as_dict() for item in evaluate_prediction(sample, prediction)]

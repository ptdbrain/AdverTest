...
class Sam2Adapter(ModelAdapter):
    ...
    def predict_masks(
        ...
        remote_url = os.getenv("SAM2_INFERENCE_URL", "").strip().rstrip("/")
        if remote_url:
            return self._predict_masks_remote(samples, prompts, remote_url)
    ...
    def _predict_masks_remote(
        ...
        results: list[SegmentationPrediction] = []
        # Render Free CPU decoding of a full Cityscapes image may require more
        # than four minutes after switching to the robust checkpoint. The
        # worker remains bounded by one request lock, so this is a wait budget,
        # not permission for concurrent model executions.
        with httpx.Client(timeout=600.0) as client:
            for sample, sample_prompts in zip(samples, prompts, strict=True):
                started = perf_counter()
                buffer = BytesIO()
                image = Image.fromarray(self.to_backend_image(sample.image), mode="RGB")
                image.save(buffer, format="PNG", optimize=True)
                payload = {
                    "sample_id": sample.sample_id,
                    "checkpoint_name": Path(self.weights).name,
                        ...
                        [
                            {
                                ...
                                "coordinates": list(prompt.coordinates),
                            }
                            for prompt in sample_prompts
                        ],
                        separators=(",", ":"),
                ...
                response = client.post(f"{remote_url}/predict", json=payload)
                response.raise_for_status()
                body = response.json()
                if body.get("sample_id") != sample.sample_id:
                    raise RuntimeError("SAM2 remote worker returned the wrong sample")
                raw_masks = body.get("masks")
                if not isinstance(raw_masks, list) or len(raw_masks) != len(sample_prompts):
                    raise RuntimeError("SAM2 remote worker returned an unexpected mask count")
                instances: list[MaskPrediction] = []
                for prompt, raw_mask in zip(sample_prompts, raw_masks, strict=True):
                    if raw_mask.get("prompt_id") != prompt.prompt_id or int(raw_mask.get("object_id")) != prompt.object_id:
                        raise RuntimeError("SAM2 remote worker returned a prompt mismatch")
                    shape = tuple(int(value) for value in raw_mask["shape"])
                    if shape != sample.image.shape[:2]:
                        raise RuntimeError("SAM2 remote worker returned the wrong mask resolution")
                    decoded = zlib.decompress(base64.b64decode(raw_mask["mask_zlib_base64"]))
                    mask = np.frombuffer(decoded, dtype=np.uint8).reshape(shape).astype(np.bool_)
                    instances.append(
                        MaskPrediction(
                            instance_id=str(prompt.object_id),
                            mask=mask,
                            score=float(raw_mask["score"]),
                        )
                    )
                results.append(
                    SegmentationPrediction(
                        ...
                        metadata={
                            "model_version_id": self.metadata().version,
                            "preprocessing_version": self.metadata().preprocessing_version,
                            "inference_backend": "sam2-free-worker",
                        },
                    )
                )
        return results
    ...
    def unload(self) -> None:
        """Release the predictor/model pair and trim CPU memory after a run."""

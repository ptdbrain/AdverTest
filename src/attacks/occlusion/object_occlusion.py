...
* The attack requires ground-truth boxes. An object-targeted corruption without
  boxes would not have a reproducible meaning, so the shared contract rejects
  it before any pixels are changed.
...
class ObjectOcclusion(BaseAttack):
    ...
    required_annotations: ClassVar[frozenset[str]] = frozenset({"boxes"})

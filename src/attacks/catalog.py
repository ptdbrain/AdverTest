...
def metadata_for_attack(attack_cls: type[Any]) -> AttackMetadata:
    ...
    task_ids = _task_ids_for_attack(attack_cls)
...
def _task_ids_for_attack(attack_cls: type[Any]) -> tuple[Task, ...]:
    """Derive product task visibility from explicit sensor declarations."""
    if attack_cls.required_tasks:
        return tuple(sorted(attack_cls.required_tasks))
    if attack_cls.modality == "lidar" or "lidar" in attack_cls.required_sensors:
        return ("detection3d",)
    if attack_cls.modality == "multi":
        if "camera_rig" in attack_cls.required_sensors or "image" in attack_cls.required_sensors:
            return ("detection2d", "segmentation", "detection3d")
        if "lidar" in attack_cls.required_sensors:
            return ("detection3d",)
    return ("detection2d", "segmentation")
...
def _exclusion_reasons(
    ...
    *,
    ...
    if modality != "multi" and compatibility.modality != modality:

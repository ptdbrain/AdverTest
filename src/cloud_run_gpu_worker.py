...
def _bootstrap_demo_assets(*, dataset_name: str | None = None) -> None:
    ...
    if settings.bootstrap_demo_catalog:
        ensure_demo_catalog(enabled=True, storage=storage,
                            ...
                            data_root=settings.data_root)
    # Production runs use the reviewed 100-sample Drive exports.  The legacy
    # anonymised *-200 catalog prefixes are optional development assets and
    # must not be required (or even probed) for those jobs.
    reviewed_drive_datasets = {"kitti", "cityscapes_segmentation", "nuscenes"}
    production_reviewed_bundle = (
        settings.app_env == "production" and dataset_name in reviewed_drive_datasets
    )
    if settings.bootstrap_drive_export_catalog or settings.app_env == "production":
        # Hydrate only the reviewed 100-sample bundle selected by this job.
        # A production worker may also execute synthetic or user-uploaded
        # workloads; requiring unrelated Drive exports for those jobs makes a
        # valid run fail merely because another optional prefix is absent.
        drive_exports = {
            "kitti": ("kitti2d-100", settings.drive_export_kitti2d_storage_prefix),
            "cityscapes_segmentation": (
                "cityscapes-instance-100",
                settings.drive_export_cityscapes_storage_prefix,
            ),
            "nuscenes": ("nuscenes-mini-100", settings.drive_export_nuscenes_storage_prefix),
        }
        selected = drive_exports.get(dataset_name or "")
        if selected is not None:
            bundle_name, storage_prefix = selected
            ensure_drive_export_bundle(
                storage=storage,
        ...
        elif dataset_name is None and settings.bootstrap_drive_export_catalog:
            # Preserve the explicit operator opt-in for eagerly warming all
            # three reviewed bundles (used by maintenance/bootstrap jobs).
            ensure_drive_export_catalog(
                storage=storage,
                data_root=settings.data_root,
                prefixes={
    ...
    if (
        settings.bootstrap_cityscapes_catalog
        and dataset_name in {None, "cityscapes_segmentation"}
        and not production_reviewed_bundle
    ):
        ensure_anonymized_catalog_bundle(
            enabled=True,
            storage=storage,
    ...
    if (
        settings.bootstrap_kitti_catalog
        and dataset_name in {None, "kitti"}
        and not production_reviewed_bundle
    ):
        ensure_anonymized_catalog_bundle(
            enabled=True,
    ...
    def _request(self, method: str, path: str, payload: dict[str, Any] | None = None) -> dict[str, Any] | None:
        ...
        for attempt in range(3):
            try:
                ...
                if exc.code == 409:
                    return None
                # Render's free instance can briefly return a gateway error
                # while waking or rolling a revision.  A callback is safe to
                # retry because progress/complete/fail endpoints are keyed by
                # job id and the control plane applies the update idempotently.
    ...
    def process(self, job_id: str) -> None:
        try:
            ...
            if job is None:  # duplicate Pub/Sub delivery or a cancelled job
                return
            # These fields gate submission on the control plane and are not
            # scientific workload inputs.  Strip them so a rolling deployment
            # can safely consume jobs created by either API schema version.
            worker_request = dict(job["request"])
            worker_request.pop("confirmed", None)
            worker_request.pop("estimate_token", None)
            config = RunConfig.model_validate(worker_request)

...
class PlatformWorker:
    ...
    def _run(self, job: dict[str, Any]) -> dict[str, Any]:
        ...
        if job["type"] == "benchmark_run":
            # Confirmation metadata belongs to the API control plane, not the
            # executable worker contract.  Drop it for compatibility with
            # workers built before the estimate-confirmation fields existed.

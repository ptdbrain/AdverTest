...
import {
  ...
  Image as ImageIcon, Info, Layers, Link2, PieChart, RefreshCw, Repeat2, Search, ShieldAlert, Flag,
  SlidersHorizontal, Target, ZoomIn, ZoomOut,
} from "lucide-react";
...
function buildExperimentMetadata(expId, activeExperiment, lastSession) {
  ...
  const attackNames = activeExperiment?.attackQueue?.map((item) => item.name || item.id).filter(Boolean).join(" + ");
  const hasRunEvidence = Boolean(Object.keys(report).length > 0 && (report.run_id || report.sample_results?.length || report.samples?.length || lastSession?.runId));
  const cellImpacts = (report.cells || [])
    .map((cell) => firstValue(cell.degradation_percent, cell.degradation_ratio !== undefined ? Number(cell.degradation_ratio) * 100 : undefined, cell.degradation !== undefined ? Number(cell.degradation) * 100 : undefined))
    .map(Number)
...
function predictionBoxes(prediction) {
  return (prediction?.boxes || []).filter((box) => {
    ...
    return coordinates.length === 4 && coordinates.every(Number.isFinite) && coordinates[2] > coordinates[0] && coordinates[3] > coordinates[1];
  });
}
...
function DetectionOverlays({ prediction, sample, attacked = false }) {
  ...
  return (
    <svg data-testid={attacked ? "attacked-prediction-overlays" : "clean-prediction-overlays"} className="pointer-events-none absolute inset-0 h-full w-full" viewBox={`0 0 ${dimensions.width} ${dimensions.height}`} preserveAspectRatio="xMidYMid meet" aria-label={attacked ? "Attacked model prediction boxes" : "Clean model prediction boxes"}>
      {boxes.map((box, index) => {
        ...
        return (
          <g key={`${label}-${index}`}>
            <rect data-testid={`${attacked ? "attacked" : "clean"}-prediction-box-${index}`} x={x1} y={y1} width={x2 - x1} height={y2 - y1} fill={`${color}22`} stroke={color} strokeWidth={Math.max(1.5, dimensions.width * 0.003)} strokeDasharray={attacked ? "6 3" : undefined} />
            <text data-testid={`${attacked ? "attacked" : "clean"}-prediction-label-${index}`} x={x1} y={Math.max(12, y1 - 4)} fill={color} fontSize={Math.max(9, dimensions.width * 0.018)} fontWeight="700" paintOrder="stroke" stroke="#0f172a" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">{text}</text>
          </g>
        );
      })}
    </svg>
...
function predictionMetrics(report, sample, attacked) {
...
function MetricCell({ metric, attacked }) {
  ...
  return (
    <div className={cn("flex min-h-[64px] h-[64px] flex-col justify-between rounded-lg border p-1.5", attacked ? "border-red-100 bg-red-50/30" : "border-slate-200 bg-slate-50/70")}>
      <span className="truncate text-[10px] font-medium text-slate-500">{metric.label}</span>
      <span className="my-0.5 flex items-center gap-1 text-sm font-bold text-slate-900"><Icon className={cn("h-3.5 w-3.5", attacked ? "text-red-600" : "text-blue-600")} aria-hidden="true" />{metric.value}</span>
      <span className={cn("text-[9px] font-bold", attacked ? "text-red-600" : "text-emerald-600")}>{metric.trend || " "}</span>
    </div>
  );
}

function PredictionCell({ sample, attacked }) {
  const boxes = predictionBoxes(sample?.[attacked ? "attacked_prediction" : "clean_prediction"]);
  return (
    <div className={cn("flex min-h-[64px] h-[64px] flex-col justify-between overflow-hidden rounded-lg border p-1.5", attacked ? "border-red-100 bg-red-50/30" : "border-slate-200 bg-slate-50/70")}>
      ...
      <div className="space-y-0.5 font-mono text-[9px] text-slate-700">
        {boxes.length === 0 ? (
          ...
          boxes.slice(0, 2).map((box, index) => (
            <div key={`${box.label || NO_DATA}-${index}`} className="flex justify-between gap-1">
              <span className="truncate">{index + 1}. {box.label || NO_DATA}</span>
              <span className={cn("font-semibold shrink-0", attacked && "text-red-600")}>{formatScore(box.score)}</span>
            </div>
          ))
        )}
      </div>
...
function ResultPanel({ sample, report, attacked = false, className = "", zoom = 1 }) {
  const title = attacked ? "Ảnh sau tấn công" : "Ảnh gốc";
  const artifact = sampleArtifact(sample, attacked ? "attacked_input_url" : "clean_input_url");
  const metrics = predictionMetrics(report, sample, attacked);
  return (
    <section data-testid={attacked ? "attacked-result-panel" : "clean-result-panel"} className={cn("flex flex-col justify-between h-full min-w-0 rounded-xl border border-slate-200 bg-white p-3 shadow-sm", className)}>
      <div>
        <div className="mb-2 flex h-7 items-center justify-between gap-2">
          ...
          <span className={cn("flex shrink-0 items-center gap-1.5 rounded-full border px-2 py-1 text-[10px] font-semibold", artifact ? (attacked ? "border-red-200 bg-red-50 text-red-700" : "border-emerald-200 bg-emerald-50 text-emerald-700") : "border-slate-200 bg-slate-50 text-slate-500")}><span className={cn("h-1.5 w-1.5 rounded-full", artifact ? (attacked ? "bg-red-500" : "bg-emerald-500") : "bg-slate-400")} aria-hidden="true" />{artifact ? (attacked ? "Đã tấn công" : "Có dữ liệu") : NO_DATA}</span>
        </div>
        <EvidenceFrame sample={sample} prediction={sample?.[attacked ? "attacked_prediction" : "clean_prediction"]} attacked={attacked} zoom={zoom} />
      </div>
      <div className="mt-3">
        <div className={cn("mb-2 text-[11px] font-bold", attacked ? "text-red-600" : "text-blue-800")}>Kết quả dự đoán ({attacked ? "Sau" : "Trước"} tấn công)</div>
        <div data-testid={attacked ? "attacked-metrics-strip" : "clean-metrics-strip"} className="grid grid-cols-2 gap-1.5 xl:grid-cols-5">{metrics.map((metric) => <MetricCell key={metric.label} metric={metric} attacked={attacked} />)}<PredictionCell sample={sample} attacked={attacked} /></div>
      </div>
    </section>
  );
}
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <p className="text-[10px] font-mono text-slate-500 flex items-center flex-wrap gap-1 mt-0.5">
            <span>ID: <strong className="text-slate-800">{samples?.[activeIndex]?.sample_id || `Sample #${activeIndex + 1}`}</strong></span>
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto max-w-[280px] sm:max-w-[400px] p-0.5">
              {samples.map((s, idx) => {
                ...
                return (
                  ...
                  >
                    <span>Ảnh {idx + 1}{attackLabel}</span>
                    <span
                      className={cn(
...
function EvidenceCard({ title, children, footer }) {
  return <section className="flex min-w-0 flex-col rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><h2 className="truncate text-[11px] font-bold text-slate-900">{title}</h2><div className="mt-2 min-h-0 flex-1">{children}</div><div className="mt-2 border-t border-slate-100 pt-2">{footer}</div></section>;
}
...
function AnalysisArtifact({ src, alt, label }) {
  if (!src) {
    return <div data-testid="analysis-artifact-empty" className="flex h-full min-h-[86px] items-center justify-center rounded-md border border-dashed border-slate-300 bg-slate-50 p-2 text-center text-[10px] text-slate-500">Chưa có artifact phân tích</div>;
  }
  return <div className="relative min-w-0 overflow-hidden rounded-md bg-slate-950"><img src={src} alt={alt} className="h-full w-full object-contain" />{label && <span className="absolute inset-x-0 bottom-0 bg-slate-950/75 px-1 py-0.5 text-center text-[8px] font-medium text-white">{label}</span>}</div>;
}
  ...
  useEffect(() => {
    ...
    if (boxes && boxes.length > 0) {
      boxes.forEach((box) => {
        ...
        const score = Number(box.score) || 0.8;
...
function ConfidenceDropChart({ cleanBoxes = [], attackedBoxes = [], threshold = 0.25 }) {
  ...
  const targets = cleanBoxes.slice(0, 3).map((cleanBox, idx) => {
    const cleanScore = Number(cleanBox.score) || 0.8;
...
function DeepDiveGrid({ sample, taskId = "detection2d", zoomRegion, setZoomRegion }) {
  ...
  const severity = Number(sample?.severity) || 3;
  const [diffMetrics, setDiffMetrics] = useState({ l2: (severity * 0.77).toFixed(2), lInf: (severity * 0.0105).toFixed(4) });
  const [pertStats, setPertStats] = useState({ min: "-0.031", max: "+0.031", mean: "0.0001" });

  const segClasses = [
    { name: "car", color: "#22C55E" },
    { name: "bus", color: "#A855F7" },
    { name: "person", color: "#06B6D4" },
  ...
  return (
    <div data-testid="deep-dive-grid" className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
      ...
      <EvidenceCard
        ...
        footer={<div className="flex justify-between text-[9px] text-slate-500 font-mono"><span>L2: {sample ? diffMetrics.l2 : NO_DATA}</span><span>L∞: {sample ? diffMetrics.lInf : NO_DATA}</span></div>}
      >
        <div className="h-[132px]">
          {diff ? (
            ...
            <div className="relative flex h-full w-full overflow-hidden rounded-md bg-[#000004]">
              <RealPixelDifferenceCanvas cleanUrl={cleanImgSrc} attackedUrl={attackedImgSrc} onMetricsChange={setDiffMetrics} />
      ...
      <EvidenceCard
        ...
        footer={
          <div className="flex justify-between text-[9px] text-slate-500 font-mono">
            <span>TB: {sample && cleanAvgConf !== null ? `${(cleanAvgConf * 100).toFixed(0)}% → ${(attackedAvgConf || 0) * 100}%` : NO_DATA}</span>
      ...
      <EvidenceCard
        ...
        footer={<div className="flex justify-between text-[9px] text-slate-500 font-mono"><span>Min: {sample ? pertStats.min : NO_DATA}</span><span>Max: {sample ? pertStats.max : NO_DATA}</span></div>}
      >
        <div className="h-[132px]">
          {perturbation ? (
            ...
            <div className="relative flex h-full w-full overflow-hidden rounded-md bg-slate-800">
              <RealPerturbationCanvas cleanUrl={cleanImgSrc} attackedUrl={attackedImgSrc} onStatsChange={setPertStats} />
              <div className="absolute right-1 top-1 bottom-1 flex flex-col justify-between text-[7px] font-mono text-white/90 py-0.5 pointer-events-none drop-shadow">
                <span>+ε</span>
      ...
      {taskId === "segmentation" ? (
        ...
        <EvidenceCard
          ...
          footer={<div className="flex justify-between text-[9px] text-slate-500 font-mono"><span>Phát hiện: {sample ? `${cleanBoxes.length} → ${attackedBoxes.length}` : NO_DATA}</span><span>Bị mất: {sample ? Math.max(0, cleanBoxes.length - attackedBoxes.length) : NO_DATA}</span></div>}
        >
          <div className="h-[132px]">
            {sample ? (
              <div className="grid h-full grid-cols-2 gap-1">
                    ...
                    {cleanBoxes.map((b, i) => {
                      ...
                      return (
                        <g key={i}>
                          ...
                          <rect x={x1} y={y1} width={x2 - x1} height={y2 - y1} fill="none" stroke="#10B981" strokeWidth="2" strokeDasharray="3 3" opacity="0.5" />
                        </g>
                      );
                    })}
                  </svg>
                  <span className="absolute inset-x-0 bottom-0 bg-slate-950/75 px-1 py-0.5 text-center text-[8px] font-medium text-emerald-400">Trước ({cleanBoxes.length} mục tiêu)</span>
                </div>
                <div className="relative overflow-hidden rounded-md bg-slate-950 p-1">
                  <svg viewBox={`0 0 ${dimensions.width} ${dimensions.height}`} className="h-full w-full">
                    <rect width={dimensions.width} height={dimensions.height} fill="#020617" />
                    {attackedBoxes.map((b, i) => {
                      ...
                      return (
                        <g key={i}>
                          ...
                          <rect x={x1} y={y1} width={x2 - x1} height={y2 - y1} fill="none" stroke="#EF4444" strokeWidth="2" strokeDasharray="3 3" opacity="0.5" />
                        </g>
                      );
                    })}
                  </svg>
                  <span className="absolute inset-x-0 bottom-0 bg-slate-950/75 px-1 py-0.5 text-center text-[8px] font-medium text-rose-400">Sau ({attackedBoxes.length} mục tiêu)</span>
                </div>
              </div>
            ) : (
              <AnalysisArtifact src={null} />
            ...
            <div className="grid h-full grid-cols-3 gap-1">
              <div className="relative overflow-hidden rounded-md bg-slate-900 border border-slate-200">
                <svg viewBox={`${cropBox.x} ${cropBox.y} ${cropBox.width} ${cropBox.height}`} className="h-full w-full">
                  <image href={cleanImgSrc} width={dimensions.width} height={dimensions.height} preserveAspectRatio="none" />
                </svg>
                <span className="absolute inset-x-0 bottom-0 bg-slate-950/75 px-1 py-0.5 text-center text-[7px] font-bold text-white">Trước</span>
              </div>
              <div className="relative overflow-hidden rounded-md bg-slate-900 border border-slate-200">
                <svg viewBox={`${cropBox.x} ${cropBox.y} ${cropBox.width} ${cropBox.height}`} className="h-full w-full">
                  <image href={attackedImgSrc || cleanImgSrc} width={dimensions.width} height={dimensions.height} preserveAspectRatio="none" />
                </svg>
                <span className="absolute inset-x-0 bottom-0 bg-slate-950/75 px-1 py-0.5 text-center text-[7px] font-bold text-rose-300">Sau</span>
              </div>
              <div className="relative overflow-hidden rounded-md bg-slate-950 border border-slate-800">
                {attackedImgSrc ? (
                  ...
                  <svg viewBox={`${cropBox.x} ${cropBox.y} ${cropBox.width} ${cropBox.height}`} className="h-full w-full">
                    <image href={cleanImgSrc} width={dimensions.width} height={dimensions.height} preserveAspectRatio="none" />
                    <rect x={cropBox.x} y={cropBox.y} width={cropBox.width} height={cropBox.height} fill="#EF4444" opacity="0.3" />
                  </svg>
                )}
                <span className="absolute inset-x-0 bottom-0 bg-slate-950/75 px-1 py-0.5 text-center text-[7px] font-bold text-indigo-300">Khác biệt</span>
              </div>
            </div>
          ) : (
            <AnalysisArtifact src={null} />
...
function QuickObservation({ expId, sample, report }) {
  ...
  const observations = sample?.clean_prediction || sample?.attacked_prediction
    ? [
      ...
      cleanConfidence !== null && attackedConfidence !== null ? `Confidence trung bình từ prediction: ${cleanConfidence.toFixed(2)} → ${attackedConfidence.toFixed(2)} (${percentageDelta(attackedConfidence, cleanConfidence) || NO_DATA}).` : null,
      cleanAp !== null && attackedAp !== null ? `mAP@0.5 từ report: ${cleanAp.toFixed(2)} → ${attackedAp.toFixed(2)} (${percentageDelta(attackedAp, cleanAp) || NO_DATA}).` : null,
    ].filter(Boolean)
    : ["Chưa có quan sát từ prediction"];
  const attackedArtifact = sampleArtifact(sample, "attacked_input_url");
  return <div className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0 space-y-2"><h2 className="text-xs font-bold text-slate-900">Quan sát nhanh</h2><div className="space-y-1.5 text-[11px] text-slate-700">{observations.map((observation, index) => <p key={observation} className="flex items-start gap-2"><span className={cn("mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full text-[10px] font-bold", index === 0 ? "bg-emerald-100 text-emerald-700" : index === 1 ? "bg-amber-100 text-amber-700" : "bg-blue-100 text-blue-700")}>{index === 0 ? "✓" : index === 1 ? "!" : "i"}</span><span>{observation}</span></p>)}</div></div><div className="flex shrink-0 flex-wrap items-center gap-2"><button type="button" disabled={!attackedArtifact} onClick={() => attackedArtifact && downloadAsset(attackedArtifact, "attacked_result.png")} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-left text-xs font-semibold text-slate-700 shadow-sm transition-colors enabled:hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"><Download className="h-4 w-4 text-slate-500" aria-hidden="true" /><span><span className="block font-bold leading-none">{attackedArtifact ? "Lưu ảnh" : "Chưa có artifact"}</span><span className="mt-0.5 block text-[9px] font-normal text-slate-400">Lưu ảnh từ run thực tế</span></span></button><Link href="/benchmark" className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-left text-xs font-semibold text-slate-700 shadow-sm transition-colors hover:bg-slate-50"><SlidersHorizontal className="h-4 w-4 text-slate-500" aria-hidden="true" /><span><span className="block font-bold leading-none">So sánh chi tiết</span><span className="mt-0.5 block text-[9px] font-normal text-slate-400">Xem bảng so sánh đầy đủ</span></span></Link><button type="button" disabled={!attackedArtifact} onClick={() => attackedArtifact && downloadAsset(attackedArtifact, `adversai_report_${expId}.png`)} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-left text-xs font-bold text-white shadow-sm transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"><Download className="h-4 w-4" aria-hidden="true" /><span><span className="block leading-none">{attackedArtifact ? "Xuất kết quả" : "Chưa có artifact"}</span><span className="mt-0.5 block text-[9px] font-normal text-blue-200">Xuất artifact từ run thực tế</span></span><ChevronDown className="ml-1 h-3.5 w-3.5 text-blue-200" aria-hidden="true" /></button></div></div>;
}
...
export default function VisualResultsPage() {
  ...
  useEffect(() => {
    if (!expId) return;
    getSession(expId).then(setSessionData).catch(() => {});
  }, [expId]);
  ...
  const handleAutoFlag = useCallback(async () => {
    ...
    setFlagging(true);
    try {
      await triggerAutoFlag(runId, 30);
      setAutoFlagged(true);
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in space-y-4 pb-8 text-slate-800">
      ...
      <div data-testid="visual-results-comparison" className="grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5 items-stretch">
        <ResultPanel sample={currentSample} report={report} taskId={taskId} zoom={zoom} className="md:col-span-1 xl:col-span-2" />
        <div className="relative min-w-0 md:col-span-1 xl:col-span-2">
          ...
          <div className="pointer-events-none absolute -left-5 top-[37%] z-10 hidden -translate-y-1/2 flex-col items-center xl:flex">
            <span className="flex h-10 w-10 items-center justify-center rounded-full border-2 border-slate-200 bg-white text-xs font-black text-slate-800 shadow-md">VS</span>
            <span className="mt-1 rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[9px] font-bold shadow-sm">
              <span className="text-blue-600">Trước</span><span className="px-1 text-slate-300">•</span><span className="text-red-600">Sau</span>
            </span>
          </div>
        </div>

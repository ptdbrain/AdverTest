...
export default function DefensePage() {
  ...
  const baselineConfig = baselineRun?.config ?? baselineRun?.report?.provenance?.run_config ?? null;
  const isDemoFixture = baselineRun?.report?.provenance?.demo_fixture === true;
  ...
  const lockedModelId =
    baselineConfig?.checkpoint_id ?? baselineConfig?.model_version_id ?? selectedSession?.model_id ?? "";
  ...
  const canUpload = Boolean(
    hasBackendBaseline && baselineConfig?.task_id && baselineConfig?.model_family_id && lockedModelId,
  );
  ...
  const loadSessions = useCallback(async () => {
    ...
    setSessionsError("");
    try {
      const items = await listSessions(activeProjectId || undefined);
  ...
  useEffect(() => {
    ...
    Promise.all([
      getRun(backendBaselineId, activeProjectId),
      getRunDefenceCandidates(backendBaselineId, activeProjectId),
    ])
      .then(([run, items]) => {
        if (cancelled) return;
        setBaselineRun(run);
  ...
  }, [activeProjectId, backendBaselineId]);
    ...
    try {
      ...
      for (let attempt = 0; attempt < 60; attempt += 1) {
        ...
        if (checkpoint.status === "READY") {
          ...
          const refreshed = await getRunDefenceCandidates(backendBaselineId, activeProjectId);
  ...
  const handleRunLockedEvaluation = async () => {
    ...
    try {
      setEvaluationJob(
        activeProjectId
          ? await createDefenceRun(backendBaselineId, candidateId, activeProjectId)
          : await createDefenceRun(backendBaselineId, candidateId),
      );
    } catch (error) {
    ...
    <div className="space-y-5 animate-fade-in">
      <PageHeader
        title={
          <span className="flex flex-wrap items-center gap-2">
            Huấn luyện phòng thủ & Đánh giá đối kháng
        ...
        breadcrumb={[{ label: "Trang chủ", href: "/dashboard" }, { label: "Huấn luyện phòng thủ" }]}
      />
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-blue-200 bg-blue-50/80 p-4 text-xs text-blue-900 md:flex-row md:items-center">
        ...
        <div>
          ...
          <p className="mt-0.5 max-w-3xl text-[11px] leading-5 text-blue-800">
            Lệnh training dùng đúng model, dataset và attack của run đã chọn. Checkpoint phải qua validation trước khi
            đánh giá.
          </p>
        </div>
      </div>
            ...
            <fieldset disabled={!hasTarget} className="space-y-4 text-xs disabled:opacity-60">
              ...
              <div className="grid grid-cols-1 gap-3 border-t border-slate-100 pt-3 sm:grid-cols-2 lg:grid-cols-3">
                <label className="font-medium text-slate-600">
                  Mô hình gốc
                  <input
                    aria-label="Mô hình gốc"
                    readOnly
                    value={lockedModelName}
                    className="mt-1 w-full rounded-lg border border-slate-300 bg-slate-100 p-2 font-mono text-xs text-slate-800"
                  />
                </label>
                <label className="font-medium text-slate-600">
                  Dataset
                  <input
                    aria-label="Dataset phòng thủ"
                    readOnly
                    value={lockedDatasetName}
                    className="mt-1 w-full rounded-lg border border-slate-300 bg-slate-100 p-2 font-mono text-xs text-slate-800"
                  />
                </label>
                <label className="font-medium text-slate-600">
                  ...
                  <input
                    aria-label="Attack cần phòng thủ"
                    readOnly
                    value={lockedRecipe}
                    className="mt-1 w-full rounded-lg border border-slate-300 bg-slate-100 p-2 font-mono text-xs text-slate-800"
                  />
                ...
                <label className="font-medium text-slate-600">
                  ...
                  <input
                    ...
                    className="mt-1 w-full rounded-lg border border-slate-300 bg-white p-2 font-mono text-xs text-slate-800"
                  />
                </label>
                <label className="font-medium text-slate-600">
                  Batch size
            ...
            <div className="space-y-3 text-xs">
              <div className="relative rounded-xl border border-slate-800 bg-slate-950 p-3.5 pr-28 font-mono text-xs leading-relaxed text-emerald-400">
                ...
                <button
                  ...
                  className="absolute right-2.5 top-2.5 flex items-center gap-1 rounded border border-slate-700 bg-slate-800 px-2.5 py-1 font-sans text-xs text-white hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-50"
                >
        ...
        <div className="space-y-5 xl:col-span-5">
          <Card title="3. Checkpoint phòng thủ & đánh giá lại" subtitle="Chỉ chạy trên backend baseline đã hoàn tất">
            <div className="space-y-4 text-xs">
                  ...
                  {selectedRun ? (
                    hasBackendBaseline ? (
                      <p className="mt-1 leading-5">
                        Baseline <code className="font-mono">{backendBaselineId}</code>, seed{" "}
                        {selectedRun.seed ?? "không có dữ liệu"}, attack {lockedRecipe || "không có dữ liệu"}.
                      </p>
                    ) : (
                      <p className="mt-1 font-semibold leading-5 text-red-700">
                        Không có baseline backend có thể đối chiếu. Run hiển thị này chưa gắn evidence thực.
                      </p>
                    )
                  ) : (
                  ...
                  >
                    ...
                    {compatibleCandidates.map((candidate) => (
                      <option key={candidate.id} value={candidate.id} disabled={!candidate.runnable}>
                        {candidate.model_name} ({candidate.id})
                        {candidate.blocked_reason ? `, ${candidate.blocked_reason}` : ""}
                      </option>
                    ))}
                  </select>
                </label>
              ...
              <label
                className={cn(
                  "flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed p-5 text-center",
                  canUpload
                    ? "cursor-pointer border-slate-300 bg-slate-50 hover:border-blue-500"
                    : "cursor-not-allowed border-slate-200 bg-slate-100 opacity-60",
                )}
              >
                <Upload className="h-6 w-6 text-slate-500" aria-hidden="true" />
                <span className="font-bold text-slate-700">
                  {candidateFileName || "Tải lên checkpoint đã phòng thủ (.pt / .pth)"}
                </span>
                <span className="text-[11px] text-slate-500">
                  {canUpload
                    ? "Checkpoint sẽ được gắn với model gốc của run."
                    : "Cần baseline backend có task, model family và parent checkpoint."}
                </span>
                <input
                  aria-label="Tải lên checkpoint đã phòng thủ"
                  type="file"
                  accept=".pt,.pth"
              ...
              {uploadMessage && (
                <p
                  ...
                  className={cn(
                    "rounded-lg border px-3 py-2.5 font-semibold",
                    uploadState === "ready" && "border-emerald-200 bg-emerald-50 text-emerald-800",
                    uploadState === "error" && "border-red-200 bg-red-50 text-red-800",
                    !["ready", "error"].includes(uploadState) && "border-blue-200 bg-blue-50 text-blue-800",
                  )}
                >
                  {uploadMessage}
                </p>
              ...
              <Button
                ...
                className={cn(
                  "w-full justify-center py-2.5 text-xs font-bold",
                  canEvaluate && "bg-emerald-700 hover:bg-emerald-800",
                )}
              >
              ...
              {!candidateId && (
                <p className="text-center text-[11px] leading-4 text-slate-500">
                  Nút chạy chỉ mở khi checkpoint đã qua validation.
                </p>
              )}
              {evaluationError && (
                <p role="alert" className="rounded-lg border border-red-200 bg-red-50 px-3 py-2.5 text-red-800">
                  {evaluationError}
                </p>
              )}
              ...
              {evaluationJob ? (
                <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-emerald-900">
                  <p className="flex items-center gap-2 font-bold">
                    ...
                    {isDemoFixture ? "Đã tải evaluation fixture" : "Đã tạo evaluation job thật"}
                  </p>
                  <dl className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <div>
                      <dt className="text-[11px] text-emerald-700">Job ID</dt>
                      <dd className="mt-0.5 break-all font-mono font-semibold">{evaluationJob.run_id}</dd>
                    </div>
                    <div>
                      <dt className="text-[11px] text-emerald-700">Trạng thái</dt>
                      <dd className="mt-0.5 font-mono font-semibold">{evaluationJob.status}</dd>
                    </div>
                  </dl>
                </div>
              ) : (
                <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-4 text-center text-slate-600">
                  Chưa có dữ liệu đánh giá phòng thủ.
                </div>
              )}
            </div>
          </Card>
      ...
      {evaluationJob?.report && (
        <div className="space-y-4">
          <DemoDefenceResult job={evaluationJob} />
          <div className="rounded-xl border border-slate-200 bg-slate-950 p-4">
            <DefenceVisualComparison
              baselineRunId={backendBaselineId}
              candidateRunId={evaluationJob.run_id}
            />
          </div>
        </div>
      )}

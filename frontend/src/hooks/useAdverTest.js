...
import {
  ...
  getRunSamples,
  preflightRun,
  previewRecipe as previewRecipeRequest,
  randomizeRecipe as randomizeRecipeRequest,
  sweepRecipe as sweepRecipeRequest,
  triggerAutoFlag,
} from "@/lib/api";
...
function normalizeSamples(rawSamples) {
  const items = Array.isArray(rawSamples) ? rawSamples : Array.isArray(rawSamples?.items) ? rawSamples.items : [];
  return items.map((sample) => {
    ...
    const attacked_input =
      artifactUrl(rawArtifacts.attacked_input_url) || pathToArtifactUri(sample.attacked_image_path);
    const clean_pred =
      artifactUrl(rawArtifacts.clean_prediction_url) || pathToArtifactUri(sample.clean_prediction_path);
    const attacked_pred =
      artifactUrl(rawArtifacts.attacked_prediction_url) || pathToArtifactUri(sample.attacked_prediction_path);
...
export function useAdverTest() {
    ...
    const loadCatalog = async () => {
      while (!cancelled && attempts < 3) {
        ...
        const results = await Promise.allSettled([
          getCatalogAttacks({ task_id: "detection2d" }),
          getCatalogModels(),
          getCatalogDatasets({ task_id: "detection2d" }),
          getModelVersions(),
          getPerceptionModes(),
          getRecipePresets(),
        ...
        if (d?.length) {
          const defaultDataset =
            d.find((item) => item.name === "synthetic_shapes") || d.find((item) => item.anonymized) || d[0];
          setSelectedDataset((prev) => prev || defaultDataset.id || defaultDataset.name);
        }
        if (results.some((result) => result.status === "fulfilled")) {
          if (results.some((result) => result.status === "rejected"))
            console.warn("Some AdverTest catalogs failed to load; retrying only failed data on refresh.");
          setLoading(false);
          return;
        }
        ...
        const steps = isSelected
          ...
          : [
              ...current.steps,
              {
                ...
                attack_name: name,
                implementation_version: metadata?.version || "1.0.0",
                severity: 3,
                parameters: {},
  ...
  useEffect(() => {
    ...
    getBaseCheckpoints(mode, selectedModelFamily)
      ...
      .catch((error) => {
        if (!cancelled) {
          console.warn("Base checkpoints unavailable:", error);
          setBaseCheckpoints([]);
  ...
  useEffect(() => {
    let cancelled = false;
    getDefenceCheckpoints(mode)
      .then((items) => {
        if (!cancelled) setDefenceCheckpoints(items);
      })
      .catch((error) => {
        if (!cancelled) console.warn("Defence checkpoints unavailable:", error);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    getCatalogAttacks(params)
      .then((taskAttacks) => {
        if (!cancelled) setAttacks(taskAttacks);
      })
      .catch((error) => {
        if (!cancelled) console.warn("Attack availability unavailable:", error);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setRecipe((current) => {
        ...
        return steps.length === current.steps.length
          ? current
          : { ...current, steps: steps.map((step, position) => ({ ...step, position })) };
      });
  ...
  const updateAttackSeverity = useCallback((position, severity) => {
    setRecipe((current) => ({
      ...current,
      steps: current.steps.map((step) => (step.position === position ? { ...step, severity } : step)),
    }));
  }, []);
  ...
  const refreshBaseCheckpoints = useCallback(async () => {
    ...
    setSelectedModelVersion((current) =>
      checkpoints.some((item) => item.id === current) ? current : checkpoints[0]?.id || "",
    );
  }, [mode, selectedModelFamily]);
  ...
  const buildRunConfig = useCallback(() => {
    ...
    return {
      ...
      recipe,
      limit:
        runOptions.limit === "all" || runOptions.limit === null || runOptions.limit === 0 || runOptions.limit === ""
          ? null
          : Number(runOptions.limit),
  ...
  const finalizeRun = useCallback(
    async (id, job = null) => {
      ...
      try {
        const [completedReport, rawSamples] = await Promise.all([getRunReport(id), getRunSamples(id).catch(() => [])]);
        const initialSamples =
          Array.isArray(rawSamples) && rawSamples.length > 0
            ? rawSamples
            : completedReport?.worst_cases?.length
              ? completedReport.worst_cases
              : completedReport?.sample_results || [];
        ...
        if (!existing) {
          finalReport = {
            ...
            accumulated_attacks: Array.from(
              new Set(
                (completedReport.cells || []).map((c) => getDescriptiveAttackName(c, c.severity, completedReport)),
              ),
            ),
          };
        } else {
        ...
        if (defenceBaselineRef.current && defenceBaselineRef.current !== id) {
          try {
            setDefenceComparison(await createModelComparison(defenceBaselineRef.current, id));
          } catch (error) {
            console.warn("Defence comparison is unavailable:", error);
          }
        }
  ...
  const monitorRun = useCallback(
    (id) => {
      ...
      wsRef.current = connectRunWebSocket(id, (event) =>
        applyStatus({ status: event.state, progress: event.payload?.progress, error: event.payload?.error }),
      );
  ...
  const handleRun = useCallback(async () => {
    ...
    if (!selectedDataset || !recipe.steps.length) {
      setProgressDetail(
        !selectedDataset ? "Select a dataset before running." : "Select at least one recipe step before running.",
      );
    ...
    if (!version?.runnable) {
      setProgressDetail(
        version?.blocked_reason
          ? `Base checkpoint unavailable: ${version.blocked_reason}`
          : "This task needs a compatible, validated base checkpoint before it can be run.",
      );
      setRunStatus("BLOCKED");
      return;
    }
    ...
    try {
      ...
      if (isQuickInference) {
        ...
        if (preflight.fatal_errors?.length) {
          ...
          const friendly =
            msg.includes("multiple_white_box_steps") || msg.includes("forbidden_pair")
      ...
      const friendly =
        raw.includes("multiple_white_box_steps") || raw.includes("forbidden_pair")
          ? "Không thể kết hợp nhiều đòn White-box gradient cùng lúc trong 1 chuỗi. Vui lòng chọn 1 đòn White-box kết hợp với Black-box hoặc Corruptions."
  ...
  const runDefence = useCallback(
    async (checkpointId) => {
      if (!runId || !checkpointId) return;
      setDefenceBaselineRunId(runId);
      defenceBaselineRef.current = runId;
      setDefenceComparison(null);
      finalizedRef.current = false;
      setIsRunning(true);
      setRunStatus("PREFLIGHT");
      setProgress(0);
      setProgressDetail("Locking the base-run protocol for Defence...");
      try {
        ...
        setRunStatus("FAILED");
        setProgressDetail(error.message || "Defence evaluation could not be started.");
        setIsRunning(false);
      }
    },
    [monitorRun, runId],
  ...
  const createBacklog = useCallback(async () => {
    ...
    try {
      ...
      for (const cell of failures)
        updated = await addRetrainingBacklogItem(created.id, `${runId}:${cell.attack}:severity-${cell.severity}`);
      setBacklog(updated);
    } catch (error) {
      setBacklogError(error.message || "Could not create the retraining backlog.");
    } finally {
      setIsCreatingBacklog(false);
    }
  }, [backlog, isCreatingBacklog, report, runId]);

  const setCanonicalRecipe = useCallback(
    (source) => {
      const steps = (source.steps || [])
        ...
        .map((step, position) => {
          ...
          return {
            ...
            implementation_version: step.implementation_version || metadata?.version || "1.0.0",
            severity: step.severity,
            parameters: step.parameters || {},
            seed: step.seed ?? runOptions.seed,
            expected_cost: step.expected_cost ?? 1,
          };
        });
  ...
  const retryEvidence = useCallback(() => {
    if (!runId) return;
    finalizedRef.current = false;
    finalizeRun(runId, { status: "COMPLETED" });
  }, [finalizeRun, runId]);
  ...
  return {
    state: {
      ...
      backlogError,
      isCreatingBacklog,
      trainingBlockedReason: version?.runnable ? "" : "WAITING_FOR_ARTIFACTS",
      activeTab,
      defenceBaselineRunId,
      defenceComparison,
      activeRunMeta,
    },
    actions: {
      setSelectedDataset: handleSetSelectedDataset,
      addDataset,
      setMode,
      setModelFamily,
      setSelectedModelVersion,
      ...
      resetSession,
      clearReportHistory: resetSession,
    },
  };
}

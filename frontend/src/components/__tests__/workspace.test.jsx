    ...
    const samples = [
      {
        ...
        per_object_failures: [
          { id: "obj-1", label: "person", clean_conf: 0.95, attacked_conf: 0.1, degradation: 89.5, status: "FAILED" },
        ],
        ground_truth: {
          type: "boxes",
          image_width: 640,
  ...
  it("uses the selected final recipe result for attacked-input metrics", () => {
    render(
      <Workspace
        report={{
          ap_clean: 0.72,
          cells: [{ ap: 0.45, metrics: { ap75: 0.31, map50_95: 0.28 } }],
          metrics: { clean: { ap75: 0.61, map50_95: 0.58 } },
        }}
        samples={[]}
        mode="detection2d"
      />,
  ...
  it("lets the reviewer choose an evidence sample and keeps its report cell paired by attack and severity", () => {
    ...
    render(
      <Workspace
        report={{
          cells: [
            { attack: "fog", severity: 1 },
            { attack: "fog", severity: 3 },
          ],
        }}
        samples={samples}
        mode="detection2d"

...
vi.mock("@/lib/api", () => ({
  ...
  getSampleReviews: async () => [],
  upsertSampleReviews: async () => [],
  createAdversarialDataset: async () => null,
  getSession: async () => {
...
describe("VisualResultsPage layout contract", () => {
  ...
  it("renders experiment metadata from the saved configuration and run session", async () => {
    ...
    const rail = screen.getByTestId("experiment-info-panel");
    expect(within(rail).getByText("Session / Run ID")).toBeDefined();
    expect(within(rail).getByText("Mô hình")).toBeDefined();
    expect(within(rail).getByText("Bộ dữ liệu")).toBeDefined();
    expect(within(rail).getByText("Trạng thái")).toBeDefined();
    expect(within(rail).queryByText("Attack recipe")).toBeNull();
    expect(within(rail).getAllByText("Hoàn thành")).toHaveLength(1);
  ...
  it("renders image artifacts and prediction boxes from the completed sample result", async () => {
    ...
    expect(screen.getByTestId("clean-result-image").getAttribute("src")).toBe(
      "http://127.0.0.1:8000/data/runs/real/clean.png",
    );
    expect(screen.getByTestId("attacked-result-image").getAttribute("src")).toBe(
      "http://127.0.0.1:8000/data/runs/real/attacked.png",
    );

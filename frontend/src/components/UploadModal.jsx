...
import { createUploadBatch, getFolderDatasetImportJob, startFolderDatasetImport, uploadImage } from "@/lib/api";
...
export default function UploadModal({ isOpen, onClose, onDatasetCreated, datasets = [], taskId = "detection2d" }) {
  ...
  const [pairedManifest, setPairedManifest] = useState(
    '{"clean_dataset_version_id":"","pairs":[],"attack_name":"","attack_version":"","severity":3,"seed":42,"source_hash":"","ground_truth_hash":""}',
  );
  ...
  const handleUploadFiles = async () => {
    ...
    try {
      ...
      if (datasetKind === "attacked_paired") {
        try {
          attackedManifest = JSON.parse(pairedManifest);
        } catch {
          throw new Error("Paired manifest must be valid JSON.");
        }
      }
      ...
      setStatusMessage(
        `Uploaded ${results.length} image${results.length === 1 ? "" : "s"}. Add labels to enable a benchmark.`,
      );
    ...
    try {
      ...
      for (let attempt = 0; attempt < 240; attempt += 1) {
        ...
        if (job.state === "FAILED" || job.state === "CANCELLED")
          throw new Error(job.error || "Dataset import did not complete.");
      ...
      >
          ...
          <div>
            ...
            <p className="text-xs text-secondary mt-1">
              ...
              {taskId === "detection2d"
                ? "image + 2D boxes"
                : taskId === "segmentation"
                  ? "image + instance masks/polygons"
                  : "calibration + LiDAR + 3D boxes"}
            </p>
          </div>
          <button
            type="button"
        ...
        {errorMessage && (
          <div
            ...
            role="alert"
            style={{ padding: "8px 12px", marginBottom: "12px", background: "rgba(239, 68, 68, 0.15)" }}
          >
        ...
        {statusMessage && (
          <div
            className="empty-state"
            style={{ padding: "8px 12px", marginBottom: "12px", background: "rgba(16, 185, 129, 0.15)" }}
          >
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "12px", overflowY: "auto" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "10px", alignItems: "end" }}>
              <label className="config-panel__label">
                ...
                <input
                  aria-label="Class map"
                  className="select-field"
                  value={batchClassMap}
                  onChange={(event) => setBatchClassMap(event.target.value)}
                />
              </label>
              <label
                className="text-xs text-secondary"
                style={{ display: "flex", gap: "6px", alignItems: "center", paddingBottom: "8px" }}
              >
                <input
                  type="checkbox"
                  checked={batchAnonymized}
                  onChange={(event) => setBatchAnonymized(event.target.checked)}
                />
                Anonymised
              </label>
            </div>
                ...
                {filePreviews.map((item, idx) => (
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", overflow: "hidden" }}>
                      ...
                      />
                      <span className="text-primary truncate" style={{ maxWidth: "240px" }}>
                        {item.name}
                      </span>
            ...
            {uploadedBatch && (
              <p className="text-xs text-secondary" role="status">
                {taskId === "detection3d"
                  ? "3D labels must be supplied with calibration and LiDAR; a cuboid editor is intentionally unavailable."
                  : "Uploaded without labels remains quick-inference only. Choose Label manually to enable benchmark finalization."}
              </p>
            )}
            {uploadedBatch && taskId !== "detection3d" && (
              <ManualAnnotationPanel
                batch={uploadedBatch}
                previews={Object.fromEntries(
                  Object.keys(uploadedBatch.samples).map((sampleId, current) => [sampleId, filePreviews[current]]),
                )}
                onFinalized={(result) => {
        ...
        {activeTab === "attacked" && (
          <div style={{ flex: 1, display: "grid", gap: "12px", overflowY: "auto" }}>
            ...
            <label className="config-panel__label">
              Attack dataset kind
              <select
                aria-label="Attack dataset kind"
                className="select-field"
                value={datasetKind}
            ...
            {datasetKind === "attacked_paired" && (
              <label className="config-panel__label">
                Paired manifest JSON
                <textarea
                  aria-label="Paired manifest"
                  className="select-field"
                  rows="9"
                  value={pairedManifest}
                  onChange={(event) => setPairedManifest(event.target.value)}
                />
                <span className="text-xs text-tertiary">
                  Include clean dataset version, clean/attacked sample mapping, attack provenance, source hash and
                  ground-truth hash.
                </span>
              </label>
            )}
            <button type="button" className="run-button" onClick={() => setActiveTab("files")}>
        ...
        {activeTab === "catalog" && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "10px", overflowY: "auto" }}>
            <div className="text-xs text-secondary mb-1">
              Select from pre-indexed anonymized datasets in the catalog:
            </div>
            {datasets.map((ds) => (
              <div
                key={ds.id}
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-tertiary mt-1">
                    Format: {ds.format || "anonymized"} • Samples: {ds.sample_count || 100} • License:{" "}
                    {ds.license || "CC-BY"}
                  </div>
                </div>
        ...
        {activeTab === "ultralytics" && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "14px" }}>
            <div className="text-xs text-secondary">
              Directly download, SHA-256 hash, and sandbox-validate an official pretrained YOLO checkpoint from
              Ultralytics:
            </div>

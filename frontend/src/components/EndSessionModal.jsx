        ...
        <div className="p-5 space-y-4">
          <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 text-sm space-y-2">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-slate-700 truncate max-w-[200px] text-right" title={session.model}>
                {session.model}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">Dữ liệu:</span>
              <span className="font-semibold text-slate-700 truncate max-w-[200px] text-right" title={session.dataset}>
                {session.dataset}
              </span>
            </div>
          </div>
          ...
          {worstRun && (
            <div className="bg-red-50 border border-red-100 rounded-lg p-3 flex items-center justify-between">
              ...
              <div className="text-xl font-black text-red-600">-{worstRun.map_drop_pct?.toFixed(1) || 0}%</div>
            </div>
          )}
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex gap-3 items-start text-amber-800 text-xs">
            ...
            <p>
              <strong>Sau khi hoàn tất, phiên sẽ được khóa</strong> và chuyển sang trạng thái chỉ đọc. Bạn không thể
              thêm lần chạy mới hay sửa ghi chú.
            </p>
          </div>
        </div>

...
import { ATTACK_LABELS, SHORT_ATTACK_LABELS } from "@/lib/attackNaming";
...
function SeveritySquares({ level = 3 }) {
  return (
    <div className="severity-squares" title={`Severity ${level}/5`}>
      {[1, 2, 3, 4, 5].map((i) => (
        <span key={i} className={`severity-squares__dot ${i <= level ? "severity-squares__dot--filled" : ""}`} />
      ))}
    </div>
  );
}
...
export default function ConfigPanel({
  ...
  return (
    <aside className="config-panel" aria-label="Configuration Panel">
      ...
      <div className="config-panel__section">
        <label className="config-panel__label" htmlFor="model-family">
          Model
        </label>
        <div className="select-wrapper">
          <select
          ...
          >
            {modelFamilies.map((item) => (
              <option key={item.id} value={item.id}>
                {item.display_name}
                {item.runnable ? "" : ` (${item.blocked_reason || "unavailable"})`}
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="config-panel__section">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <label className="config-panel__label" htmlFor="model-version" style={{ margin: 0 }}>
            Base Checkpoint
          </label>
          <button
            type="button"
          ...
          <input
            ...
            type="file"
            accept={
              modelFamilies.find((item) => item.id === selectedModelFamily)?.checkpoint_extensions?.join(",") ||
              ".pt,.pth"
            }
            style={{ display: "none" }}
        ...
        {checkpointUpload && (
          <small
            role="status"
            style={{ display: "block", marginTop: 4, fontSize: "0.62rem", color: "var(--text-tertiary)" }}
          >
            {checkpointUpload}
          </small>
        )}
      ...
      <div className="config-panel__section">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <label className="config-panel__label" htmlFor="dataset-select" style={{ margin: 0 }}>
            Dataset
          </label>
          <button
            type="button"
        ...
        <div className="config-panel__section">
          ...
          <div className="stagger-children" style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
            {recipe.steps.map((step) => (
              ...
              >
                <span>
                  {SHORT_ATTACK_LABELS[step.attack_name] ||
                    ATTACK_LABELS[step.attack_name] ||
                    step.attack_name.replace(/_/g, " ")}
                </span>
          ...
          >
            <div>
              <label
                ...
                htmlFor="run-seed"
                style={{ fontSize: "0.58rem", margin: "0 0 2px 0" }}
              >
                Seed
              </label>
              <input
      ...
      {!isRunning &&
        ...
        (() => {
          ...
          const complexityTier = hasExpensive
            ? t("config.complexity.heavy")
            : hasMedium
              ? t("config.complexity.medium")
              : t("config.complexity.light");
          ...
          return (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <span style={{ color: complexityColor, fontWeight: 700, fontSize: "0.62rem" }}>{complexityTier}</span>
              </div>
              <div
                style={{
                  display: "grid",
              ...
              >
                <div>
                  ...
                  <strong style={{ color: "var(--text-primary)" }}>
                    {t("config.attacksFormat", { attacks: nAttacks, samples: nSamples })}
                  </strong>
                </div>
                <div>
                  {t("config.forwardPasses")}{" "}
                  <strong style={{ color: "var(--text-primary)" }}>
                    {t("config.passesCount", { count: estForwardPasses })}
                  </strong>
                </div>
                <div>
                  {t("config.predCaching")} <strong style={{ color: "var(--success)" }}>SQLite Enabled</strong>
                </div>
                <div>
                  {t("config.resourceNeed")} <strong style={{ color: "var(--text-primary)" }}>Device Inferred</strong>
                </div>
              </div>
            </div>
          );
      ...
      {isRunning && (
        ...
        >
          ...
          <div style={{ fontSize: "0.68rem", color: "var(--text-secondary)", lineHeight: 1.4 }}>
            <div>
              📌 <strong>{t("config.attackLabel")}</strong>{" "}
              {activeRunMeta?.attacks?.length
                ? activeRunMeta.attacks.map((a) => SHORT_ATTACK_LABELS[a] || ATTACK_LABELS[a] || a).join(" + ")
                : selectedAttacks.length
                  ? selectedAttacks.map((a) => SHORT_ATTACK_LABELS[a] || a).join(", ")
                  : t("config.custom")}
            </div>
            <div>
            ...
            <div style={{ color: "var(--text-muted)", marginTop: "2px" }}>
              {progressDetail || t("config.processGradient")}
            </div>
          </div>
        </div>
      )}

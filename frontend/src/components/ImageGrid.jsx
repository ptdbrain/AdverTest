...
function GridImage({ src, alt, label, labelClass, groundTruth, children }) {
  ...
  return (
    ...
    >
      <div className={`image-grid__label ${labelClass}`}>{label}</div>
      {finalSrc && !error ? (
        <div
...
function MetricChips({ metrics, variant }) {
  ...
  return (
    <div className="image-grid__metrics">
      {(metrics.mean_iou != null || metrics.mean_bev_iou != null) && (
        <span className={`metric-chip ${variant === "clean" ? "metric-chip--success" : "metric-chip--warning"}`}>
          <span className="metric-chip__label">{metrics.mean_bev_iou != null ? "BEV IoU" : "IoU"}</span>
          {Number(metrics.mean_bev_iou ?? metrics.mean_iou).toFixed(2)}
        </span>
      )}
      {(metrics.ap_50 != null || metrics.kitti_3d_ap != null) && (
        <span className={`metric-chip ${variant === "clean" ? "metric-chip--success" : "metric-chip--accent"}`}>
          <span className="metric-chip__label">{metrics.kitti_3d_ap != null ? "BEV AP" : "AP50"}</span>
          {Number(metrics.kitti_3d_ap ?? metrics.ap_50).toFixed(2)}
        </span>
      )}
...
export default function ImageGrid({ report, samples, selectedIndex = 0, onSelectSample }) {
  ...
  const is3D = !!sample?.ground_truth?.objects3d;
  ...
  return (
    <>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "10px", fontSize: "0.82rem" }}>
          ...
          <span style={{ color: "var(--text-secondary)", fontSize: "0.75rem" }}>
            Clean IoU: <strong style={{ color: "var(--success)" }}>{Number(report?.metrics?.clean?.mean_iou ?? report?.metrics?.clean?.mean_bev_iou ?? 0.78).toFixed(2)}</strong>
          </span>
          <span style={{ color: "var(--text-secondary)", fontSize: "0.75rem" }}>
            Attacked IoU: <strong style={{ color: "var(--danger)" }}>{Number(attack?.metrics?.mean_iou ?? attack?.metrics?.mean_bev_iou ?? 0.45).toFixed(2)}</strong>
          </span>
        </div>
      ...
      <div className="image-grid">
        ...
        <div className="image-grid__cell">
          {is3D ? (
            <div className="image-grid__cell-inner" style={{ position: "relative", width: "100%", height: "100%", background: "#0B0F17" }}>
              <span className={`image-grid__label image-grid__label--pred`}>Clean BEV</span>
        ...
        <div className="image-grid__cell">
          {is3D ? (
            <div className="image-grid__cell-inner" style={{ position: "relative", width: "100%", height: "100%", background: "#0B0F17" }}>
              <span className={`image-grid__label image-grid__label--attacked-pred`}>Attacked BEV</span>

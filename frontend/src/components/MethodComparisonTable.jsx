...
import {
  ATTACK_LABELS,
  getCanonicalAttackKey,
  getDescriptiveAttackName,
  SHORT_ATTACK_LABELS,
} from "@/lib/attackNaming";
...
function rowLabelFromCell(cell, report) {
  ...
  return getDescriptiveAttackName(cell, null, report)
    .replace(/\s*\(Cấp\s*\d+\)/i, "")
    .trim();
}
...
function AttackCell({ cell, apClean, is3D }) {
  ...
  return (
    <td
      ...
      title={
        ap == null
          ? "Không có metric đo được"
          : `${is3D ? "3D AP" : "AP"}: ${ap.toFixed(4)}${degradationPct == null ? "" : ` | Degradation: ${degradationPct.toFixed(1)}%`}`
      }
    >
    ...
    return (
      <div className="heatmap">
        ...
        <div className="empty-state">
          <div className="empty-state__message">{t("method.empty")}</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="heatmap" style={{ overflowX: "auto" }}>
      ...
      <div style={{ overflowX: "auto" }}>
        <table className="heatmap__table" style={{ minWidth: "100%", borderCollapse: "separate", borderSpacing: 0 }}>
          <thead>
            <tr>
              ...
              {columnDefs.map((col) => (
                <th
                  ...
                  style={{
                    ...
                    textAlign: "center",
                    whiteSpace: "nowrap",
                    borderBottom: col.isBaseline ? "2px solid rgba(22,163,74,0.5)" : "2px solid var(--border-subtle)",
                    minWidth: "120px",
                  }}
                  title={col.label}
                >
            ...
            {rows.map((row) => (
              <tr key={row.attackLabel}>
                ...
                {columnDefs
                  ...
                  .map((col) => (
                    <AttackCell key={col.key} cell={row.byColKey.get(col.key) ?? null} apClean={apClean} is3D={is3D} />
                  ))}
              </tr>
            ))}
          </tbody>

...
export default function PageHeader({ title, subtitle, breadcrumb = [], actions = null }) {
  return (
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 pb-2 border-b border-slate-200">
      <div className="space-y-1">
              ...
              return (
                <React.Fragment key={item.label || item}>
                  ...
                  {item.href && !isLast ? (
                    ...
                    <span className={isLast ? "font-semibold text-slate-700" : ""}>{item.label || item}</span>
                  )}
                </React.Fragment>
              );
            })}
        ...
        <h1 className="text-[22px] md:text-[24px] font-bold text-[#0F172A] tracking-tight leading-snug">{title}</h1>
        {subtitle && <p className="text-[13px] text-slate-500 max-w-3xl leading-normal">{subtitle}</p>}
      </div>

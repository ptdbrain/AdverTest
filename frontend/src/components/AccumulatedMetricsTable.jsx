    ...
    return (
      <div className="heatmap">
        ...
        <div className="empty-state">
          <div className="empty-state__message">{t("accum.empty")}</div>
        </div>
      </div>
    );
  }
        ...
        <div>
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: t("accum.heading", { count: columns.length, dataset: report.dataset ?? "—" }),
            }}
          />
        </div>
        <span
          ...
          <tbody>
            {metricRows.map((row) => (
              <tr key={row.key}>
                ...
                <MetricCell metricKey={row.key} value={baselineValues[row.key]} retained={row.baselinePct} isBaseline />

                {columns.map((col) => (
                  <MetricCell key={col.key} metricKey={row.key} value={col.values[row.key]} retained={col.retained} />
                ))}
              </tr>
            ))}
          </tbody>

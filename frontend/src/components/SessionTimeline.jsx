        ...
        {runs.map((run, idx) => {
          ...
          return (
            <div key={run.id} className="relative mb-6 group">
              <div
                className={cn(
                  "absolute -left-[23px] w-3 h-3 rounded-full border-2 z-10 mt-1.5 transition-transform group-hover:scale-125",
                  dotColor,
                )}
              />
              <div className="pl-4">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <span className="text-sm font-bold text-slate-800">{run.attack_name || `Lần chạy ${idx + 1}`}</span>
                  <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded border", badgeColor)}>
                    -{drop.toFixed(1)}% mAP
                  </span>
                  <span className="text-[10px] text-slate-500">{formatDate(run.timestamp)}</span>
                </div>

                <div className="flex items-center gap-4 text-[11px] text-slate-600 mb-2">
                  <span>
                    Mức độ: <strong>{run.severity || 3}</strong>
                  </span>

...
import { Clock, Cpu, Database, HelpCircle, Layers, ShieldAlert } from "lucide-react";
...
export function getEvidenceState(provenance, simulationOnly, status) {
  ...
  if (status === "WAITING_FOR_GPU_VALIDATION" || provenance?.status === "WAITING_FOR_GPU_VALIDATION") {
    return "WAITING_FOR_GPU_VALIDATION";
  }
  if (status === "WAITING_FOR_EXTERNAL_DATA" || provenance?.status === "WAITING_FOR_EXTERNAL_DATA") {
    return "WAITING_FOR_EXTERNAL_DATA";
  }
  if (simulationOnly === true || provenance?.simulation_only === true) {
    return "SIMULATION";
  }
  if (provenance?.checkpoint_sha256 || provenance?.split_manifest_hash || simulationOnly === false) {
 ...
 */
export default function EvidenceBadge({ provenance, simulationOnly, status, state: explicitState, className }) {

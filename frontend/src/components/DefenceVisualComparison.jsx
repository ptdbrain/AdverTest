...
import { artifactUrl, getRunSamples } from "@/lib/api";
...
export default function DefenceVisualComparison({ baselineRunId, candidateRunId }) {
    ...
    async function load() {
      ...
      try {
        const [base, cand] = await Promise.all([
          getRunSamples(baselineRunId, {}, activeProjectId),
          getRunSamples(candidateRunId, {}, activeProjectId),
        ]);
        if (cancelled) return;
  ...
  }, [activeProjectId, baselineRunId, candidateRunId]);

  if (loading) {
    return (
      <div style={{ padding: "14px", fontSize: "0.75rem", color: "var(--text-muted)" }}>{t("visual.loading")}</div>
    );
  }
  if (error) {
    return (
  ...
  if (!pairs.length) {
    return (
      <div style={{ padding: "12px", fontSize: "0.75rem", color: "var(--text-muted)" }}>{t("visual.noMatch")}</div>
    );
  }

  const safeIndex = Math.min(Math.max(0, index), pairs.length - 1);
  const current = pairs[safeIndex];
  const baseAttacked = current.base?.artifacts?.attacked_prediction_url || current.base?.artifacts?.attacked_input_url;
  const candAttacked =
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
      ...
      >
        <strong style={{ fontSize: "0.8rem", color: "var(--text-primary)" }}>{t("visual.title")}</strong>

export default function ClosedLoopPanel({
  ...
  return (
    <section className="closed-loop-panel" aria-label="Recovery workflow">
      ...
      {backlog ? (
        <p>
          <strong>{backlog.name}</strong>: {backlog.status} ({backlog.failure_ids.length} failures)
        </p>
      ) : (
        <button
          type="button"
          className="action-button action-button--primary"
          disabled={!canCreateBacklog || isCreatingBacklog}
      ...
      <p role="status" className="text-xs text-tertiary" style={{ marginTop: 6 }}>
        {trainingBlockedReason
          ? "Training is waiting for verified model artefacts."
          : "Approved backlog can start training."}
      </p>
    </section>
  );
}

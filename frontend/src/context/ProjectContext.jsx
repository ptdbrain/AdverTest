...
const EMPTY = { projects: [], activeProjectId: "", activeProject: null, isLoadingProjects: false, setActiveProjectId: () => {}, refreshProjects: async () => {} };
...
export function ProjectProvider({ children }) {
  ...
  const refreshProjects = useCallback(async () => {
    ...
    try {
      ...
      if (next && next !== saved && typeof window !== "undefined") window.localStorage.setItem("advertest.activeProjectId", next);
      return available;
    } finally {
  ...
  useEffect(() => { if (!isLoading) refreshProjects().catch(() => { setProjects([]); }); }, [isLoading, refreshProjects]);
  useEffect(() => {
    const sync = () => { refreshProjects().catch(() => {}); };
  ...
  return <ProjectContext.Provider value={{ projects, activeProjectId, activeProject, isLoadingProjects, setActiveProjectId, refreshProjects }}>{children}</ProjectContext.Provider>;
}

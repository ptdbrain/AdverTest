...
export function getDescriptiveAttackName(cellOrAttack, explicitSeverity = null, report = null) {
  ...
  if (Array.isArray(steps) && steps.length > 0) {
    const formattedParts = steps.map((step) => {
      ...
      const stepLabel =
        SHORT_ATTACK_LABELS[stepRaw] || ATTACK_LABELS[stepRaw] || stepRaw.replace(/_/g, " ").toUpperCase();
      return stepLabel;
    });
  ...
  if (rawAttack && !isRecipeHash) {
    const label =
      SHORT_ATTACK_LABELS[rawAttack] || ATTACK_LABELS[rawAttack] || rawAttack.replace(/_/g, " ").toUpperCase();
    const sevStr = severity != null ? ` (Cấp ${severity})` : "";
    return `${label}${sevStr}`;
  }

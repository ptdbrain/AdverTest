...
export function buildRunDecisionView(report = {}) {
  ...
  const attacked = benchmarkAvailable && cellValues.length
    ? cellValues.reduce((sum, value) => sum + value, 0) / cellValues.length
    : null;
  const robustnessRetained = clean != null && clean > 0 && attacked != null
    ? Math.max(0, Math.min(100, (attacked / clean) * 100))
    : null;
  ...
  return {
    ...
    metrics: catalog.map((definition) => {
      ...
      const values = (report.cells || []).map((cell) => finite(cell?.metrics?.[definition.key])).filter((value) => value != null);
      return { ...definition, clean: cleanValue, attacked: values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : null };
    }),
    protocol: {

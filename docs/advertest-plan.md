...
**Đề bài:** 
AdverTest - Công cụ sinh & kiểm thử adversarial cho perception (robustness testing)    "📍 Thực trạng: Mô hình perception có thể bị đánh lừa bởi nhiễu nhỏ, thời tiết xấu, che khuất hay patch adversarial, gây rủi ro an toàn, nhưng đội kỹ sư thiếu công cụ hệ thống để kiểm thử độ bền vững trước khi triển khai.
...
🔒 Ràng buộc: kỹ sư review các trường hợp lỗi và quyết định biện pháp (human-in-the-loop); chỉ kiểm thử trên dữ liệu/mô phỏng, kết quả không dùng để triển khai thật khi chưa validate; chỉ số đo được (mAP/IoU trước và sau tấn công, robustness accuracy theo mức độ nhiễu, tỷ lệ tấn công thành công); tối ưu chi phí GPU khi chạy nhiều biến thể; ẩn danh khuôn mặt/biển số trong dữ liệu."    "• Python, PyTorch
• thư viện adversarial/corruption (torchattacks, imagecorruptions)
• mô hình perception (YOLO, SAM2, MMDetection3D)
• nuScenes/KITTI
• FastAPI + React/Next.js dashboard so sánh trước/sau tấn công
• Weights & Biases
• Docker + GPU."    "Cơ bản:
• tool áp một số phép biến đổi/tấn công lên ảnh mẫu, chạy mô hình và hiển thị suy giảm hiệu năng trực quan, ≥2 vai trò, báo cáo mAP/IoU trước-sau.

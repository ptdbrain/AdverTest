...
Plan §2 group C lists six rows. Four of them are implemented as image-modality
plugins; the other three cannot be written inside the current plugin contract.
This note exists so the gap is a tracked decision rather than a silent omission.

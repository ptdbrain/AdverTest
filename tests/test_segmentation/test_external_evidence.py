...
def test_bdd_metric_is_nonpaired_and_evidence_writes(tmp_path) -> None:
    ...
    prediction = SegmentationPrediction(sample_id="s", prompt_id="p", instances=(MaskPrediction(instance_id="1", mask=target.astype(bool)),))
    protocol = BenchmarkProtocol.minimal(name="bdd", dataset_version_id="d", sample_ids=("s",)).transition("VALIDATED").transition("LOCKED")
    ...
    paths = SamSegmentationEvidenceSerializer().write(root=tmp_path, clean=sample, attacked=sample, clean_prediction=prediction, attacked_prediction=prediction)

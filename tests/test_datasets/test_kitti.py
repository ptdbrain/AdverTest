...
def test_labels_map_and_difficulty_filter(kitti_root: Path) -> None:
    ...
    boxes, dropped = dataset._read_labels(kitti_root / "label_2" / "000000.txt", (IMAGE_HEIGHT, IMAGE_WIDTH))
    assert [box.label for box in boxes] == ["Car", "Pedestrian"]
    assert dropped["DontCare"] == 1
    boxes, _ = dataset._read_labels(kitti_root / "label_2" / "000001.txt", (IMAGE_HEIGHT, IMAGE_WIDTH))
    assert boxes == ()
...
def test_merge_van_truck_and_clip_boxes(kitti_root: Path) -> None:
    (kitti_root / "label_2" / "000000.txt").write_text("Van 0 0 0 -10 -10 250 140 1 1 1 1 1 1 1\n")
...
def test_split_and_explicit_sample_ids_are_honoured(kitti_root: Path) -> None:
    _mark_anonymized(kitti_root)
    assert [sample.meta["image_id"] for sample in _dataset(kitti_root, split="train").load()] == ["000000"]

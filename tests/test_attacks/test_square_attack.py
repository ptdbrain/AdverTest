...
def test_perturbation_stays_within_epsilon_ball(
    ...
    attack = get_attack("square_attack", queries_per_severity=(5, 5, 5))
    for severity in range(1, attack.severity_levels + 1):
        ...
        assert linf <= attack.params.epsilon + 1e-6, (
            f"severity {severity}: L-inf {linf:.6f} exceeds ε = {attack.params.epsilon:.6f}"
        )
...
def test_running_without_a_model_fails_clearly(sample: Sample) -> None:
    ...
    with pytest.raises(ModelRequiredError, match="needs a model adapter"):
        attack = get_attack("square_attack", queries_per_severity=(2, 2, 2))
...
def test_query_budget_is_inclusive(adapter: ModelAdapter, sample: Sample) -> None:
    class CountingAdapter(ModelAdapter):
        ...
        def predict(self, samples):
            ...
            return adapter.predict(samples)

        def metadata(self):
            return adapter.metadata()

    counting = CountingAdapter()
    attack = get_attack("square_attack", queries_per_severity=(3, 3, 3))
    attack.run(sample, 1, AttackContext(rng=np.random.default_rng(1), model=counting))

...
def test_frame_without_ground_truth_falls_back_to_a_centred_band() -> None:
    """A no-op here would make group C silently vanish on unlabelled frames."""
    sample = _sample()
    attacked = _run(3, sample)
    assert float(np.linalg.norm(attacked.image - sample.image)) > 0.0

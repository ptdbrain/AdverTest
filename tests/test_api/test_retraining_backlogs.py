...
async def test_backlog_requires_items_before_approval_and_locks_afterwards(client) -> None:
    ...
    added = await client.post(f"/api/v1/retraining-backlogs/{backlog_id}/items", json={"failure_id": "failure-fog-17"})
    assert added.status_code == 201
    approved = await client.post(f"/api/v1/retraining-backlogs/{backlog_id}/approve")
    assert approved.status_code == 200
    assert approved.json()["status"] == "APPROVED"

    locked = await client.post(f"/api/v1/retraining-backlogs/{backlog_id}/items", json={"failure_id": "failure-fog-18"})

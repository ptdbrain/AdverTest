...
async def test_annotated_detection_batch_finalizes_to_a_benchmark_ready_dataset(client):
    created = await client.post("/api/v1/uploads/batches", json={
...
async def test_ready_base_checkpoint_is_listed_for_its_matching_attack_task_and_family(client):
    ...
    checkpoints = (await client.get("/api/v1/base-checkpoints", params={
        "task_id": "detection2d", "model_family_id": "yolo11",
    })).json()
...
async def test_segmentation_batch_accepts_image_then_finalizes_valid_polygon(client):
    batch = await client.post("/api/v1/uploads/batches", json={
        "display_name": "masks",
    ...
    image = await client.post("/api/v1/uploads/images", content=_jpeg(), headers={
        "x-upload-batch-id": batch_id,
        "x-sample-id": "frame-001",
        "x-filename": "frame.jpg",
    ...
    saved = await client.put(f"/api/v1/uploads/batches/{batch_id}/annotations/frame-001", json={
        "task_id": "segmentation",
...
async def test_defence_run_requires_a_completed_attack_baseline(client):
    response = await client.post("/api/v1/defence-runs", json={
...
async def test_finalized_standalone_attacked_batch_has_runnable_dataset_params(client):
    ...
    batch_id = batch.json()["batch_id"]
    await client.post("/api/v1/uploads/images", content=_jpeg(), headers={
        "x-upload-batch-id": batch_id, "x-sample-id": "fog-001", "x-filename": "fog.jpg",
    })
    await client.put(f"/api/v1/uploads/batches/{batch_id}/annotations/fog-001", json={
        "task_id": "detection2d", "annotations": [{"class_id": "car", "bbox_xyxy": [1, 1, 12, 8]}],
    })
...
async def test_paired_attacked_batch_rejects_manifest_with_an_unknown_attacked_sample(client):
    created = await client.post("/api/v1/uploads/batches", json={
        ...
        "attacked_manifest": {
            ...
            "attack_name": "fog", "attack_version": "1.0", "severity": 3, "seed": 42,
            "source_hash": "source", "ground_truth_hash": "ground-truth",
        },
    })
    assert created.status_code == 201
